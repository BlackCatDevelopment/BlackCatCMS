 ...
 */
$s_ = sha1( microtime() );
$s = substr( $s_, 0, mt_rand( 5, 61 ) );
$p = substr( $s_, 0, mt_rand( 5, 61 ) );
...
define("CONFIG_LOGIN_USERNAME", $s );
define('CONFIG_LOGIN_PASSWORD', $p );
...
//User Permissions
define('CONFIG_OPTIONS_DELETE', (isset($_SESSION['SYSTEM_PERMISSIONS']) && array_search('media_delete', $_SESSION['SYSTEM_PERMISSIONS']) !== false));
define('CONFIG_OPTIONS_CUT', (isset($_SESSION['SYSTEM_PERMISSIONS']) && array_search('media_upload', $_SESSION['SYSTEM_PERMISSIONS']) !== false));
define('CONFIG_OPTIONS_COPY', (isset($_SESSION['SYSTEM_PERMISSIONS']) && array_search('media_upload', $_SESSION['SYSTEM_PERMISSIONS']) !== false));
define('CONFIG_OPTIONS_NEWFOLDER', (isset($_SESSION['SYSTEM_PERMISSIONS']) && array_search('media_create', $_SESSION['SYSTEM_PERMISSIONS']) !== false));
define('CONFIG_OPTIONS_RENAME', (isset($_SESSION['SYSTEM_PERMISSIONS']) && array_search('media_rename', $_SESSION['SYSTEM_PERMISSIONS']) !== false));
define('CONFIG_OPTIONS_UPLOAD', (isset($_SESSION['SYSTEM_PERMISSIONS']) && array_search('media_upload', $_SESSION['SYSTEM_PERMISSIONS']) !== false)); 
define('CONFIG_OPTIONS_EDITABLE', (isset($_SESSION['SYSTEM_PERMISSIONS']) && array_search('media_upload', $_SESSION['SYSTEM_PERMISSIONS']) !== false));
//FILESYSTEM CONFIG
	/*
...
define('CONFIG_SYS_DEFAULT_PATH', '../../../../../'.MEDIA_DIRECTORY.'/'); //accept relative path only
define('CONFIG_SYS_ROOT_PATH', '../../../../../'.MEDIA_DIRECTORY.'/');	//accept relative path only
...
define("CONFIG_SYS_PATTERN_FORMAT", 'list'); //three options: reg ,csv, list, this option define the parttern format for the following patterns
	 ...
	 */
/**
 *	Try to get a valid list of allowed filetypes from the setting-table
 *
 */
$temp_query = "SELECT `value` from `".TABLE_PREFIX."settings` where `name`='rename_files_on_upload'";
$temp_result = $database->get_one( $temp_query );
if ($temp_result) {
	$lepton_file_types = $temp_result;
} else {
	$lepton_file_types = "";
}
...
define('CONFIG_SYS_EXC_DIR_PATTERN', '.svn,.git'); //will prevent listing of folders with such pattern(s). separated by , if multiple

define('CONFIG_SYS_INC_FILE_PATTERN', $lepton_file_types); //force listing of fiels with such pattern(s). separated by , if multiple

define('CONFIG_SYS_EXC_FILE_PATTERN', 'index.php,.DS_Store'); //will prevent listing of files with such pattern(s). separated by , if multiple
...
define('CONFIG_UPLOAD_VALID_EXTS', $lepton_file_types );// // 
...
define("CONFIG_VIEWABLE_VALID_EXTS", $lepton_file_types );
...
$file_name = DIR_LANG . secureFileName( $language ).".php";
if ( true === file_exists( $file_name ) ) {
	define('CONFIG_LANG_DEFAULT', secureFileName( $language ) );
} else {
