...
class Compiler implements ICompiler
{
     ...
     */
    public function getTemplateSource(mixed $fromPointer = false)
    {
        if ($fromPointer === true) {
    ...
    public function compile(Core $core, ITemplate $template) : string
    {
        ...
        while (true) {
            // if pointer is at the beginning, reset everything, that allows a
            // plugin to externally reset the compiler if everything must be reparsed
            if ($ptr === 0) {
                // resets variables
                ...
                $this->debug('===== COMPILER INIT =====' . "\n");
                $this->debug('> PROCESSING PREPROCESSORS (' . count($this->processors['pre']) . ')' . "\n");
                ...
                $this->debug("> ".print_r(htmlentities($tpl), true)."\n");
        ...
        $this->debug('> PROCESSING POSTPROCESSORS' . "\n");
        ...
        $this->debug('> COMPILATION COMPLETE : MEM USAGE : ' . memory_get_usage() . "\n");
    ...
    {
        $this->debug('===== Compiler::' . __FUNCTION__ . " =====\n");
    ...
    /**
     * validates the keys in the param array as PHP 8 passes them as
     * named params to call_user_func_array
     *
     * @access protected
     * @param  array
     * @param  mixed
     * @return mixed
     **/
    protected function processAction(array $funccompiler, mixed $params) {
        $classname = $funccompiler[0];
        $methodname = $funccompiler[1];
        // setup a reflection method object
        $refMeth = new ReflectionMethod($classname.'::'.$methodname);
        // get a list of the parameter names, if you want to go the 'named' route
        foreach($refMeth->getParameters() as $param) {
            $sigParams[$param->getName()] = 1;
        }
        // the first param should always be the compiler object
        // sigParams (example): Array ( [0] => compiler [1] => value [2] => rest )
        foreach($params as $n => $p) {
            if($n=='*') {
                if(array_key_exists('rest',$sigParams)) {
                    $params['rest'] = $p;
                    unset($params[$n]);
                }
            }
    ...
    protected function parse(string $in, int $from, ?int $to, mixed $parsingParams = false, ?string $curBlock = '', mixed &$pointer = null)
    {
        ...
        while ($first === ' ' || $first === "\n" || $first === "\t" || $first === "\r") {
            if ($curBlock === 'root' && substr($in, $from, strlen($this->rd)) === $this->rd) {
                ...
                $this->debug('===== TEMPLATE PARSING ENDED' . " =====\n");
                return false;
            }
            ++ $from;
            if ($pointer !== null) {
        ...
        $this->debug('> PARSE CALL : PARSING "' . htmlentities(substr($in, $from, min($to - $from, 50))) . (($to - $from) > 50 ? '...' : '') . '" @ ' . $from . ':' . $to . ' in ' . $curBlock . ' : pointer=' . $pointer . "\n");
        ...
        if ($parsed === 'var') {
            if (preg_match('#^\s*([/%+*-])\s*([a-z0-9]|\$)#i', $substr, $match)) {
                $this->debug('> PARSING POST-VAR EXPRESSION ' . $substr . "\n");
                // parse expressions
                ...
                $this->debug('> PARSING POST-VAR ASSIGNMENT ' . $substr . "\n");
                // parse assignment
                $value    = $match[2];
                ...
                $this->debug('> VARIABLE NAMED PARAM (FOR ARRAY) FOUND' . "\n");
                $len = strlen($match[1]);
                $var = $out[count($out) - 1];
                $pointer += $len;
    ...
    protected function parseFunction(string $in, int $from, ?int $to, mixed $parsingParams = false, ?string $curBlock = '', mixed &$pointer = null)
    {
                    ...
                    while ($ptr < strlen($paramstr)) {
                        while (true) {
                            ...
                            if ($func !== 'if' && $func !== 'elseif' && $paramstr[$ptr] === ')') {
                                $this->debug('PARAM PARSING ENDED, ")" FOUND, POINTER AT ' . $ptr . "\n");
                                break 2;
                            } elseif ($paramstr[$ptr] === ';') {
                                ++ $ptr;
                                $this->debug('PARAM PARSING ENDED, ";" FOUND, POINTER AT ' . $ptr . "\n");
                                ...
                                $this->debug('PARAM PARSING ENDED, RIGHT DELIMITER FOUND, POINTER AT ' . $ptr . "\n");
                                break 2;
                            }
                        ...
                        $this->debug('FUNC START PARAM PARSING WITH POINTER AT ' . $ptr . "\n");
                        ...
                        $this->debug('PARAM PARSED, POINTER AT ' . $ptr . ' (' . substr($paramstr, $ptr - 1, 3) . ')' . "\n");
                    }
                }
        ...
        if ($pointer !== null) {
            ...
            $this->debug('FUNC ADDS ' . ((isset($paramstr) ? strlen($paramstr) : 0) + (')' === $paramsep ? 2 : ($paramspos === false ? 0 : 1)) + strlen($func)) . ' TO POINTER' . "\n");
        }
        ...
        elseif ($pluginType & Core::CLASS_PLUGIN || ($pluginType & Core::FUNC_PLUGIN && $pluginType & Core::CLASS_PLUGIN)) {
            if ($pluginType & Core::COMPILABLE_PLUGIN) {
                ...
                $output = $this->processAction($funcCompiler,$params);
            } else {
    ...
    protected function parseString(string $in, int $from, ?int $to, mixed $parsingParams = false, ?string $curBlock = '', mixed &$pointer = null)
    {
        ...
        $this->debug('STRING FOUND (in ' . htmlentities(substr($in, $from, min($to - $from, 50))) . (($to - $from) > 50 ? '...' : '') . ')' . "\n");
        ...
        $this->debug('STRING DELIMITED: ' . substr($in, $from, $strend + 1 - $from) . "\n");
    ...
    protected function parseVar(string $in, int $from, ?int $to, mixed $parsingParams = false, ?string $curBlock = '', mixed &$pointer = null)
    {
        ...
        if (preg_match($regex, $substr, $match)) {
            ...
            if ($this->debug) {
                if ($hasMethodCall) {
                    $this->debug('> METHOD CALL FOUND : $' . $key . substr($methodCall, 0, 30) . "\n");
                } else {
                    $this->debug('> VAR FOUND : $' . $key . "\n");
                }
            }
            ...
            if ($cnt > 0) {
                ...
                $this->debug('> RECURSIVE VAR REPLACEMENT : ' . $key . "\n");

                $key = $this->flattenVarTree($parsed);

                $this->debug('> RECURSIVE VAR REPLACEMENT DONE : ' . $key . "\n");

                $output = preg_replace('#(^""\.|""\.|\.""$|(\()""\.|\.""(\)))#', '$2$3', '$this->readVar("' . $key . '")');
            } else {
                $this->debug('> parseVarKey'."\n");
    ...
    {
        $this->debug('===== parseVarKey '.$key." =====\n");
        if ($key === '') {
            $this->debug('> key is empty'."\n");
            return '$this->scope';
        }
        if (substr($key, 0, 1) === '.') {
            $key = 'dwoo' . $key;
        ...
        foreach ($tree as $bit) {
            if (is_array($bit)) {
                ...
                if (substr($key, 0, 1) === '$') {
                    ...
                    $this->debug('PARSING SUBVARS IN : ' . $key . "\n");
    ...
    {
        $pos = 0;
        $this->debug('STRING VAR REPLACEMENT : ' . $string . "\n");
        ...
        while (($pos = strpos($string, '$', $pos)) !== false) {
            ...
            $pos += strlen($var[1]) + 2;
            $this->debug('STRING VAR REPLACEMENT DONE : ' . $string . "\n");
        }
    ...
    {
        $this->debug('PARSING MODIFIERS : ' . $m[3] . "\n");
        ...
        while (strlen($cmdstrsrc) > 0 && $continue) {
            ...
            if ($cmdstrsrc[0] === ' ' || $cmdstrsrc[0] === ';' || substr($cmdstrsrc, 0, strlen($this->rd)) === $this->rd) {
                $this->debug('MODIFIER PARSING ENDED, RIGHT DELIMITER or ";" FOUND' . "\n");
                $continue = false;
                if ($pointer !== null) {
            ...
            if ($paramspos === false) {
                ...
                $params    = array();
                $this->debug('MODIFIER (' . $func . ') CALLED WITH NO PARAMS' . "\n");
            } else {
                $paramstr = substr($cmdstr, $paramspos + 1);
                if (substr($paramstr, - 1, 1) === $paramsep) {
                    $paramstr = substr($paramstr, 0, - 1);
                ...
                $params = array();
                while ($ptr < strlen($paramstr)) {
                    $this->debug('MODIFIER (' . $func . ') START PARAM PARSING WITH POINTER AT ' . $ptr . "\n");
                    $this->debug($paramstr . '--' . $ptr . '--' . strlen($paramstr) . '--modifier' . "\n");
                    $params = $this->parse($paramstr, $ptr, strlen($paramstr), $params, 'modifier', $ptr);
                    $this->debug('PARAM PARSED, POINTER AT ' . $ptr . "\n");

                    if ($ptr >= strlen($paramstr)) {
                        $this->debug('PARAM PARSING ENDED, PARAM STRING CONSUMED' . "\n");
                        break;
                    }

                    if ($paramstr[$ptr] === ' ' || $paramstr[$ptr] === '|' || $paramstr[$ptr] === ';' || substr($paramstr, $ptr, strlen($this->rd)) === $this->rd) {
                        $this->debug('PARAM PARSING ENDED, " ", "|", RIGHT DELIMITER or ";" FOUND, POINTER AT ' . $ptr . "\n");
                        if ($paramstr[$ptr] !== '|') {
                            $continue = false;
                            if ($pointer !== null) {
    ...
    protected function debug(string $msg)
    {
        ...
        if(!is_resource($this->debugfh)) {
            $this->debugfh = fopen(__DIR__.'/../cache/debug.log','w');
        }
        fwrite($this->debugfh, $msg);
    }   // end function debug()
