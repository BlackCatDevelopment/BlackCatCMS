...
if ($aid == '') {
	...
	$add_vars = array('URL_HELP_FILE' => $url_help, 'URL_ADMIN_TOOL' => $url_admintools, 'CLASS_SHOW_FTP_INFO' => 'hidden');
...
echo "is_writeable(",$addon['file'],"): ", is_writeable($addon['file']), "<br />";
		...
		$tpl->set_var(array('ADDON_NAME' => $addon['name'], 'CLASS_ODD_EVEN' => ($$addon_var % 2) ? 'odd ' : '', 'CLASS_PERMISSION' => is_writeable($addon['file']) ? '' : 'permission', 'URL_EDIT_ADDON' => $url_admintools . '&amp;aid=' . $addon_id, 'TXT_EDIT_ADDON_FILE' => $LANG['ADDON_FILE_EDITOR'][1]['TXT_EDIT_' . "{$addon_type}" . '_FILES'] . ((is_writeable($addon['file'])) ? '' : $LANG['ADDON_FILE_EDITOR'][1]['TXT_FTP_SUPPORT']), 'URL_ZIP_ADDON' => $url_mod_path . '/code/download.php?aid=' . $addon_id, 'TXT_ZIP_ADDON_FILES' => $LANG['ADDON_FILE_EDITOR'][1]['TXT_ZIP_' . "{$addon_type}" . '_FILES'], 'URL_ICON_FOLDER' => $url_icon_folder));
	...
	$tpl->set_var(array('ADDON_NAME' => $_SESSION['addon_list'][$aid]['name'], 'ADDON_PATH' => $addon_main_path, 'TXT_HELP' => $LANG['ADDON_FILE_EDITOR'][1]['TXT_HELP'], 'URL_HELP_FILE' => $url_help, 'TXT_ADDON_TYPE' => $LANG['ADDON_FILE_EDITOR'][2]['TXT_' . strtoupper($_SESSION['addon_list'][$aid]['type'])], 'URL_ICON_FOLDER' => $url_icon_folder, 'TXT_CANCEL' => $TEXT['CANCEL'], 'CLASS_HIDDEN' => ($_SESSION['addon_list'][$aid]['type'] == 'language' && ! isset($_GET['list_all'])) ? 'hidden' : '', 'TXT_FTP_NOTICE' => $LANG['ADDON_FILE_EDITOR'][1]['TXT_FTP_NOTICE'], 'CLASS_SHOW_FTP_INFO' => 'hidden', 'URL_EDIT_ADDON' => $url_admintools, 'URL_RELOAD' => $url_admintools . '&amp;aid=' . $aid . '&amp;reload', 'URL_CREATE_FILE_FOLDER' => $url_action_handler . '?aid=' . $aid . '&amp;action=4', 'URL_UPLOAD_FILE' => $url_action_handler . '?aid=' . $aid . '&amp;action=5', ));
	...
	foreach ($_SESSION['addon_file_infos'] as $index => $file_info) {
		...
		$tpl->set_var(array('FILE_NAME' => $file_name, 'FILE_SIZE' => ($file_info['size'] == '') ? '&nbsp;' : $file_info['size'], 'FILE_MAKE_TIME' => $file_info['maketime'], 'CLASS_ODD_EVEN' => ($index % 2 == 0) ? 'odd ' : '', 'CLASS_FOLDER' => ($file_info['type'] == 'folder') ? 'folder ' : '', 'CLASS_PERMISSION' => is_writeable($file_info['path']) ? '' : 'permission', 'URL_ICON_FOLDER' => $url_icon_folder, 'FILE_ICON' => $file_info['icon'], 'TXT_FILE_TYPE' => $LANG['ADDON_FILE_EDITOR'][2]['TXT_FILE_TYPE_' . strtoupper($file_info['icon'])], 'HIDE_EDIT_ICON' => ($icon_edit_url == '-') ? 'hidden' : '', 'TXT_EDIT' => ($icon_edit_url <> '-' && $file_info['icon'] == 'image') ? ($LANG['ADDON_FILE_EDITOR'][2]['TXT_EDIT'] . ' (Online: PIXLR)') : $LANG['ADDON_FILE_EDITOR'][2]['TXT_EDIT'], 'TARGET_BLANK' => ($icon_edit_url <> '-' && $file_info['icon'] == 'image') ? ' target="_blank"' : '', 'URL_EDIT_FILE' => $icon_edit_url, 'URL_RENAME_FILE' => $url_action_handler . '?aid=' . $aid . '&amp;fid=' . $index .
			'&amp;action=2', 'URL_DELETE_FILE' => $url_action_handler . '?aid=' . $aid . '&amp;fid=' . $index . '&amp;action=3'));
