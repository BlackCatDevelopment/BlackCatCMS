...
// remove old files
if( ( $dirh = opendir(WB_PATH.'/temp') ) !== false ) {
    while( ( $f = readdir($dirh) ) !== false ) {
        if( ! preg_match( '~^autosave_~i', $f ) ) continue;
        if( filemtime(WB_PATH.'/temp/'.$f) <= time()-60*60 ) unlink(WB_PATH.'/temp/'.$f);
    }
    closedir($dirh);
}
...
$temp_file = WB_PATH.'/temp/autosave_'.time().'.txt';
