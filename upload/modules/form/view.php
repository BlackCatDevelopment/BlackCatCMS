...
if($use_captcha) { ?>
	...
	<div class="field_title verification"><?php echo $TEXT['VERIFICATION']; ?>:</div>
	<div class="call_captcha"><?php call_captcha(); ?></div>
	...
	if(isset($_SESSION['form_submission_id']) AND isset($_POST['submission_id']) AND $_SESSION['form_submission_id'] == $_POST['submission_id']) {
		...
		if($required != array()) {
			if(!isset($MESSAGE['MOD_FORM_REQUIRED_FIELDS'])) {
				echo '<div class="missingfields">You must enter details for the following fields';
			} else {
				echo $MESSAGE['MOD_FORM_REQUIRED_FIELDS'];
			}
			echo ':</div><br /><ul class="missingfields">';
