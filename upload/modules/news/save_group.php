...
if($admin->get_post('title') == '')
{
	$admin->print_error(
		...
		CAT_URL.'/modules/news/modify_group.php?page_id='.$page_id.'&section_id='.$section_id.'&group_id='.$group_id
	);
}
else
{
...
// Update row
$database->query("UPDATE ".CAT_TABLE_PREFIX."mod_news_groups SET title = '$title', active = '$active' WHERE group_id = '$group_id'");
...
if(isset($_FILES['image']['tmp_name']) AND $_FILES['image']['tmp_name'] != '')
{
	...
	$new_filename = CAT_PATH.MEDIA_DIRECTORY.'/.news/image'.$group_id.'.jpg';
	...
	make_dir(CAT_PATH.MEDIA_DIRECTORY.'/.news');
	// Upload image
    ...
    {
		// Resize the image
		$thumb_location = CAT_PATH.MEDIA_DIRECTORY.'/.news/thumb'.$group_id.'.jpg';
...
if(isset($_POST['delete_image']) AND $_POST['delete_image'] != '')
{
    ...
    {
		unlink(CAT_PATH.MEDIA_DIRECTORY.'/.news/image'.$group_id.'.jpg');
	}
}
