...
$query_content = $database->query("SELECT * FROM ".CAT_TABLE_PREFIX."mod_news_groups WHERE group_id = '$group_id'");
$fetch_content = $query_content->fetchRow();

/**
 * Use images?
 */
$post_header = $database->get_one("SELECT `post_header` FROM `".CAT_TABLE_PREFIX."mod_news_settings` WHERE `section_id` = '$section_id'");
if($post_header) {	
...
<form name="modify" action="<?php echo CAT_URL; ?>/modules/news/save_group.php" method="post" enctype="multipart/form-data" style="margin: 0;">
...
<tr>
	...
	<?php if(file_exists(CAT_PATH.MEDIA_DIRECTORY.'/.news/image'.$group_id.'.jpg')) { ?>
	<td>
		<a href="<?php echo CAT_URL.MEDIA_DIRECTORY; ?>/.news/image<?php echo $group_id; ?>.jpg" title="<?php echo $TEXT['VIEW']; ?>" target="_blank">
		<img class="image_preview" src="<?php echo CAT_URL.MEDIA_DIRECTORY; ?>/.news/image<?php echo $group_id; ?>.jpg" alt="<?php echo $TEXT['VIEW']; ?>" />		
		</a>
		&nbsp;
