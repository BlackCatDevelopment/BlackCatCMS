...
if (!class_exists("wblib\wbLang", false)) {
    class wbLang
    {
        ...
        {
            $language = self::getLang(...func_get_args());
            if (!$language) {
                $language = "en_gb";
            }
            if (!array_key_exists($language, self::$instances)) {
                self::log(
                    sprintf("creating new instance with name [%s]", $language),
                    7
                );
        ...
        public static function addFile($file, $path = null, $var = null)
        {
            ...
            if (!empty($path)) {
                array_unshift(
                    self::$defaults["search_paths"],
                    self::path($path)
                );
                self::$defaults["search_paths"] = array_unique(
                    self::$defaults["search_paths"]
                );
            }
            ...
            foreach (self::$defaults["search_paths"] as $path) {
                foreach (
                    [$file, strtoupper($file), strtolower($file)]
                    as $case
                ) {
                    if (file_exists(self::path($path . "/" . $case))) {
                        self::log(
                            sprintf(
                                "found language file [%s] in path [%s]",
                                $case,
                                $path
                            ),
                            7
        ...
        public static function addPath($path, $var = null)
        {
            ...
            if (file_exists($path) && is_dir($path)) {
                ...
                // error log
                self::log(
                    sprintf("not found (or not a directory): [%s]", $path),
                    2
                );
            }
            self::log("< addPath()", 7);
        ...
        public static function checkFile($file, $check_var, $check_only = false)
        {
            ...
            if (!isset(self::$_loaded[$file]) || self::$_loaded[$file] != 1) {
                try {
                    ...
                    if (isset(${$check_var})) {
                        self::log(
                            sprintf("require passed, [%s] is set", $check_var),
                            7
                        );
                        $isIndexed =
                            array_values(${$check_var}) === ${$check_var};
                        if ($isIndexed) {
                        ...
                        if ($check_only) {
                            ...
                            self::log(
                                sprintf(
                                    "loaded language file [%s], [%d] strings added",
                                    $file,
                                    count(${$check_var})
                                ),
                                7
         ...
         **/
        public static function lang($msg, $attr = [])
        {
         ...
         * convenience function; shortcut to translate()
         **/
        public static function t($msg, $attr = [])
        {
            return self::translate($msg, $attr);
        } // end function t()
         ...
         * @return string
         **/
        public static function translate($msg, $attr = [])
        {
            ...
            if (
                self::$defaults["case_insensitive"] &&
                array_key_exists($msg, self::$_strings_lo)
            ) {
