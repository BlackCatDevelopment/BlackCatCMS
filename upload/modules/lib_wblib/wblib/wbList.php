...
*/
         ...
         *
         **/
        public static function buildList($list, $options = [])
        {
            ...
            $hidden = isset(self::$defaults["__hidden_key"])
                ? self::$defaults["__hidden_key"]
                : "";
            $type = isset($options["type"]) ? $options["type"] : "";
            $maxlevel = isset($options["maxlevel"])
                ? $options["maxlevel"]
                : self::$defaults["maxlevel"];
            $startlevel = isset($options["startlevel"])
                ? $options["startlevel"]
                : 0;
            ...
            foreach ($list as $item) {
                ...
                if (isset($item[$l_key]) && $item[$l_key] > $maxlevel) {
                    continue;
                } // skip items outside max level
            ...
            self::log(
                sprintf(
                    "start level [%s], max. level [%s], total item count [%s]",
                    $startlevel,
                    $maxlevel,
                    count($list)
                ),
            ...
            self::log(
                "children list: " .
                    str_replace(" ", "    ", var_export($children, 1)),
                7
            );
            ...
            self::log(
                sprintf(
                    "loop      [%s] (if this is empty there will be no list!)",
                    $loop
                ),
                7
            );
            ...
            while (
                ...
                elseif (!empty($children[$option["value"][$id_key]])) {
                    ...
                    if (!isset($item[$l_key]) || $item[$l_key] >= $startlevel) {
                        if ($type != "select") {
                            if (
                                !isset($item[self::$defaults["__children_key"]])
                            ) {
         ...
         * @return string
         **/
        public static function buildSelect($list, $options = [])
        {
            if (!count(self::$defaults)) {
                self::reset();
            }
            $options["type"] = "select";
            ...
            if (isset($options["options_only"]) && $options["options_only"]) {
                ...
                foreach (array_values($output) as $item) {
                    $return[$item[self::$defaults["__id_key"]]] =
                        str_repeat(
                            $space,
                            $item[self::$defaults["__level_key"]]
                ...
                foreach (array_values($output) as $item) {
                    $items[] = self::selectOption(
                        ...
                        $item[self::$defaults["__id_key"]] == $sel
                            ? 'selected="selected" '
                            : "",
                        isset($item[self::$defaults["__level_key"]])
                            ? str_repeat(
                                $space,
                                $item[self::$defaults["__level_key"]]
                            )
         ...
         * @return string
         **/
        public static function listStart($ul_id = null, array $options = [])
        {
            self::log(
                sprintf(
                    "listStart() ID [%s] open list count [%s]",
                    $ul_id,
                    self::$open_lists
                ),
                7
            ...
            if (self::$open_lists == 0) {
                ...
                $classes = [self::$defaults["top_ul_class"]];
            } else {
                $tpl = self::$defaults["list_open"];
                $classes = [self::$defaults["ul_class"]];
            }

            self::log(
                sprintf("using tpl [%s], css class [%s]", $tpl, $classes[0]),
                7
            );
            ...
            $output =
                str_repeat(self::$defaults["space"], self::$open_lists) .
                str_replace(
                    ["%%id%%", "%%class%%"],
                    [$ul_id, implode(" ", $classes)],
                    $tpl
                );
        ...
        public static function listEnd()
        {
            ...
            return str_repeat(self::$defaults["space"], self::$open_lists) .
                $output;
        ...
        {
            $li_id = self::$defaults["create_li_id"]
                ? self::$defaults["li_id_prefix"] . self::getID()
                : null;
            self::log(
                sprintf(
                    "itemStart() li id [%s] css classes [%s]",
                    $li_id,
                    $css
                ),
                7
         ...
         * @return string
         **/
        public static function getListItemCSS(array $item = [])
        {
            ...
            $options = [
                "level" => isset($item[self::$defaults["__level_key"]])
                    ? $item[self::$defaults["__level_key"]]
                    : 0,
                "in_trail" => isset($item[self::$defaults["__trail_key"]])
                    ? $item[self::$defaults["__trail_key"]]
                    : false,
                "is_selected" => isset($item[self::$defaults["__current_key"]])
                    ? $item[self::$defaults["__current_key"]]
                    : false,
                "is_open" => isset($item[self::$defaults["__is_open_key"]])
                    ? $item[self::$defaults["__is_open_key"]]
                    : false,
                "has_children" => isset(
                    $item[self::$defaults["__children_key"]]
                )
                    ? $item[self::$defaults["__children_key"]]
                    : false,
            ...
            if (
                isset(self::$defaults["li_class"]) &&
                strlen(self::$defaults["li_class"])
            ) {
                $classes[] =
                    self::$defaults["css_prefix"] . self::$defaults["li_class"];
            }
            // special CSS class for each level?
            if (
                isset(self::$defaults["create_level_css"]) &&
                self::$defaults["create_level_css"] &&
                isset(self::$defaults["li_class"]) &&
                strlen(self::$defaults["li_class"])
            ) {
                $classes[] =
                    self::$defaults["css_prefix"] .
                    self::$defaults["li_class"] .
                    "_level_" .
                    $options["level"];
            }
            // first element
            if (
                self::$isfirst &&
                isset(self::$defaults["first_li_class"]) &&
                strlen(self::$defaults["first_li_class"])
            ) {
                $classes[] =
                    self::$defaults["css_prefix"] .
                    self::$defaults["first_li_class"];
            }
            // element is in trail
            if (
                isset($options["in_trail"]) &&
                $options["in_trail"] &&
                isset(self::$defaults["trail_li_class"]) &&
                strlen(self::$defaults["trail_li_class"])
            ) {
                $classes[] =
                    self::$defaults["css_prefix"] .
                    self::$defaults["trail_li_class"];
            }
            // element has children
            if (
                isset($options["has_children"]) &&
                $options["has_children"] &&
                isset(self::$defaults["has_child_li_class"]) &&
                strlen(self::$defaults["has_child_li_class"])
            ) {
                $classes[] =
                    self::$defaults["css_prefix"] .
                    self::$defaults["has_child_li_class"];
            }
            // element is open
            if (
                isset($options["is_open"]) &&
                $options["is_open"] &&
                isset(self::$defaults["is_open_li_class"]) &&
                strlen(self::$defaults["is_open_li_class"])
            ) {
                $classes[] =
                    self::$defaults["css_prefix"] .
                    self::$defaults["is_open_li_class"];
            }
            // markup for current element
            if (
                isset($options["is_selected"]) &&
                $options["is_selected"] &&
                isset(self::$defaults["current_li_class"]) &&
                strlen(self::$defaults["current_li_class"])
            ) {
                $classes[] =
                    self::$defaults["css_prefix"] .
                    self::$defaults["current_li_class"];
            }
            // last element
            if (
                isset($options["is_last"]) &&
                $options["is_last"] &&
                isset(self::$defaults["last_li_class"]) &&
                strlen(self::$defaults["last_li_class"])
            ) {
                $classes[] =
                    self::$defaults["css_prefix"] .
                    self::$defaults["last_li_class"];
            }
         ...
         * @return
         **/
        public static function getListItemText(array $item = [])
        {
            $text = isset($item[self::$defaults["__title_key"]])
                ? $item[self::$defaults["__title_key"]]
                : null;
            if (isset($item[self::$defaults["__href_key"]])) {
                ...
                if (
                    ...
                    $text = str_replace(
                        ...
                        [
                            $item[self::$defaults["__href_key"]],
                            strlen(self::$defaults["link_class"])
                                ? "class=" . self::$defaults["link_class"]
                                : "",
                            $text,
                        ],
                        self::$defaults["href"]
        ...
        private static function selectStart($name = null, $level = null)
        {
            return str_replace(
                ["%%id%%", "%%"],
                [$name, self::$defaults["select_class"]],
         ...
         * @return string
         **/
        private static function selectOption(
            $value,
            $sel,
            $space,
            $text,
        ...
        public static function buildRecursion(&$items, $min = -9)
        {
            ...
            foreach ($items as $id => &$node) {
                ...
                // sub node
                    ...
                    if (
                        !isset($items[$node[$pk]][$ck]) ||
                        !is_array($items[$node[$pk]][$ck])
                    ) {
                        $items[$node[$pk]][$ck] = [];
                    }
            ...
            while (
                ...
                elseif (!empty($children[$option["value"][$id_key]])) {
                    if (!isset($option["value"]["__children"])) {
                        $option["value"]["__children"] = count(
                            $children[$option["value"][$id_key]]
                        );
                    }
                    $return[] = $option["value"];
        ...
        public static function reset()
        {
            ...
            self::$defaults = [
                ...
                "item" => "%%text%%",
                "select_open" =>
                    '<select name="%%id%%" id="%%id%%" class="%%">',
                "select_close" => "</select>",
                "select_option" =>
                    '<option class="%%" value="%%value%%"%%selected%%>%%text%%</option>',
