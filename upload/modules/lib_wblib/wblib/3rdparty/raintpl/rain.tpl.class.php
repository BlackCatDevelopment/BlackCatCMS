...
class RainTPL{
	...
	// -------------------------
        ...
        /**
         * added by B. Martinovic: allow to set a language helper
         *
         * @var object
         **/
        static $langh  = NULL;
        ...
        $loop_level = 0;
	 	...
	 	while( $html = array_shift( $parsed_code ) ){
            ...
            elseif ( preg_match( '~\{lang="(.*)"\}~', $html, $code ) ) {
                if(is_object(self::$langh)) {
                    if(substr_count($code[1],'|')==2)
                        $code[1] = '[['.$code[1].']]';
                    $compiled_code .=   self::$langh->translate($code[1]);
                }
                else {
                    $compiled_code .=   $code[1];
                }
            }
