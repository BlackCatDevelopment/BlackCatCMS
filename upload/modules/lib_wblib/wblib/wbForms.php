...
if (!class_exists("wblib\wbFormsBase", false)) {
    class wbFormsBase
    {
        ...
        protected static $globals = [
            ...
            "add_buttons" => true,
            "blank_span" =>
                '<span class="fbblank" style="display:inline-block;width:16px;">&nbsp;</span>',
            "css_prefix" => null,
            "enable_hints" => true,
            "fallback_path" => null,
            ...
            "path" => null,
            "required_span" =>
                '<span class="fbrequired" style="display:inline-block;vertical-align:top;width:16px;color:#b94a48;" title="%s">*</span>',
            "token" => null,
            "token_lifetime" => null,
            "var" => "FORMS",
            "wblib_url" => null,
        ...
        public function __call($method, $args)
        {
            ...
            if (method_exists($this, $method)) {
                self::log(
                    sprintf("< __call() - calling method [%s]", $method),
                    7
                );
        ...
        public static function lang()
        {
            ...
            if (!self::$wblang && !self::$wblang == -1) {
                ...
                try {
                    ...
                    $language = defined("LANGUAGE")
                        ? LANGUAGE
                        : self::$wblang->current();
                    self::$wblang = \wblib\wbLang::getInstance();
                    self::log(
                        sprintf(
                            "wbLang loaded, current language [%s]",
                            $language
                        ),
                        7
                    );
                    ...
                    if (
                        isset(self::$globals["lang_path"]) &&
                        is_dir(self::path(self::$globals["lang_path"]))
                    ) {
                        self::log(
                            sprintf(
                                "adding global lang path [%s]",
                                self::$globals["lang_path"]
                            ),
                            7
                        );
                        ...
                        if (
                            ...
                            self::log(
                                sprintf("adding file [%s]", $language),
                                7
                            );
                    ...
                    if (isset($caller["file"])) {
                        ...
                        if (
                            isset($caller["args"]) &&
                            isset($caller["args"][0]) &&
                            is_scalar($caller["args"][0]) &&
                            file_exists($caller["args"][0])
                        ) {
                            if (
                                is_dir(
                                    self::path(
        ...
        public static function log($message, $level = 3)
        {
            ...
            if (!self::$analog && !self::$analog == -1) {
                if (
                    file_exists(
                        dirname(__FILE__) .
                            "/3rdparty/Analog/wblib.init.inc.php"
                    )
                ) {
                    include_once dirname(__FILE__) .
                        "/3rdparty/Analog/wblib.init.inc.php";
                    wblib_init_3rdparty(
                        dirname(__FILE__) . "/debug/",
                        "wbForms",
                        $class::$loglevel
                    );
                    self::$analog = true;
        ...
        protected static function getURL()
        {
            $class = get_called_class();
            if (
                isset($class::$globals["wblib_url"]) &&
                $class::$globals["wblib_url"] != ""
            ) {
        ...
        protected function replaceAttr()
        {
            ...
            if (
                isset($this->attr["id"]) &&
                $class::$id_prefix &&
                substr($this->attr["id"], 0, strlen($class::$id_prefix)) !=
                    $class::$id_prefix
            ) {
            ...
            if (!isset($this->attr["class"])) {
                $this->attr["class"] = $class::$cssclass
                    ? $class::$cssclass
            ...
            if (
                ...
                isset(wbForms::$ERRORS[$this->attr["name"]])
            ) {
                $this->attr["class"] .= " ui-state-highlight";
                $this->attr["notes"] =
                    '<br /><span class="fbnote fberror">' .
            ...
            foreach (array_keys($this->attributes) as $attr) {
                if (
                    isset($this->attr[$attr]) &&
                    $this->attr[$attr] != "" &&
                    !is_array($this->attr[$attr])
                ) {
                    $replace[] = "%" . $attr . "%";
            ...
            if (!is_scalar($output)) {
                self::log(
                    sprintf(
                        'ERROR: $output is not a scalar! (class [%s])',
                        $class
                    ),
                    7
                );
                ...
                $output = str_ireplace($replace, $with, $output);
            ...
            return $output .
                ...
                !$this instanceof wbFormsElementRadiogroup
                    ? ($this instanceof wbFormsElement &&
                    isset($this->attr["type"]) &&
        ...
        public function addOption($key, $value = null)
        {
            ...
            if (
                isset($this->attr["options"]) &&
                is_array($this->attr["options"])
            ) {
                if ($value) {
...
if (!class_exists("wblib\wbForms", false)) {
    ...
    class wbForms extends wbFormsBase
    {
        ...
        public static function getInstance()
        {
            ...
            if (
                ...
                filemtime($tokenfile) < time() - 24 * 60 * 60
            ) {
        ...
        public static function loadFile(
            ...
            if (isset(self::$LOADED[$file])) {
                ...
                try {
                    ...
                    self::log(
                        sprintf(
                            "unable to load the file, exception [%s]",
                            $e->getMessage()
                        ),
                        3
                    );
        ...
        public static function addElement(
            ...
            if (!self::hasElement($elem["name"], $find_in)) {
                ...
                array_splice($array_ref[$name], $i, 0, [$elem]); //insert

                // 'create' element
                if (isset($elem["type"]) && $find_in == "ELEMENTS") {
                    self::log(
                        sprintf("creating element of type [%s]", $elem["type"]),
                        7
                    );
                    $classname =
                        "\wblib\wbFormsElement" .
                        ucfirst(strtolower($elem["type"]));
                ...
                if (
                    !isset($array_ref[$name]) ||
                    !count($array_ref[$name]) ||
                    $i == -1
                ) {
                    self::log("adding at bottom", 7);
                    self::log(
                        sprintf(
                            "array [%s] current count [%s] key [%s]",
                            $find_in,
                            count($array_ref[$name]),
                            key($array_ref[$name])
                        ),
        ...
        public function createHoneypots($count = 1)
        {
            ...
            if (!$name) {
                self::hint(
                    "[createHoneypots()] No form set; you need to use <tt>setForm(&lt;NAME&gt;)</tt> first!"
                );
         ...
         * @return
         **/
        private static function getInsertPosition(
            $where,
            $ref,
            $elem_name = null
        ) {
         ...
         * @return
         **/
        public static function moveElement(
            $elem_name,
            $insert_after,
            $ref = "ELEMENTS"
        ) {
            ...
            if ($insert_after && $insert_after !== "") {
                ...
                if (!$n) {
                    if (in_array($insert_after, ["top", "bottom"])) {
                        $pos = $insert_after;
                    } else {
                        if (
                            is_numeric($insert_after) &&
                            count(self::${$ref}[$name]) <= $insert_after
                        ) {
                            $n = $insert_after;
                        }
                    }
        ...
        public static function setForm($name = "default")
        {
            ...
            if (isset(self::$FORMS[$name])) {
                ...
                if (!count(self::$FORMS[$name])) {
                    self::log(
                        sprintf("Required form [%s] has no elements!", $name),
                        4
                    );
        ...
        public static function removeElement($elem_name, $find_in = "ELEMENTS")
        {
            ...
            if (self::hasElement($elem_name, $find_in)) {
                ...
                self::log(
                    sprintf(
                        "removing element [%s] from [%s] at position [%s]",
                        $elem_name,
                        $find_in,
                        $index
                    ),
        ...
        public function getForm($name = null, $return = true)
        {
            ...
            if (!$name) {
                self::hint(
                    "[getForm()] No form set; you need to use <tt>setForm(&lt;NAME&gt;)</tt> first!"
                );
            ...
            // CSRF token
            wbFormsProtect::$config["token_lifetime"] =
                self::$globals["token_lifetime"];
            $elements[] = self::addElement(
                wbFormsProtect::getToken(self::$globals["token"])
            )->render();
            ...
            if (!self::hasButton() && self::$globals["add_buttons"]) {
                $elements[] = wbFormsElementSubmit::get([
                    "label" => "Submit",
                ])->render();
            }
            ...
            if (count($elements)) {
                if (!$return) {
                    echo wbFormsElementForm::get()->render(
                        implode("", $elements)
                    );
                } else {
                    return wbFormsElementForm::get()->render(
                        implode("", $elements)
                    );
                }
            }
        ...
        public static function getElement($name)
        {
            ...
            self::log(
                sprintf(
                    "searching for element with name [%s], form [%s]",
                    $name,
                    $formname
                ),
                7
        ...
        public static function getElements(
            ...
            if (isset($array_ref[$formname]) && count($array_ref[$formname])) {
                if ($ignore_hidden || $ignore_labels) {
                    ...
                    self::log(
                        sprintf(
                            "< getElements() - [%s] elements returned (filtered)",
                            count($elem)
                        ),
                        7
                    );
                ...
                self::log(
                    sprintf(
                        "< getElements() - [%s] elements returned",
                        count($array_ref[$formname])
                    ),
                    7
                );
        ...
        public static function hasElement($name, $find_in = "ELEMENTS")
        {
            ...
            self::log(
                sprintf(
                    "hasElement() - checking for element [%s] in form [%s], array [%s]",
                    $name,
                    $formname,
                    $find_in
                ),
            ...
            if (isset($array_ref[$formname]) && count($array_ref[$formname])) {
                foreach ($array_ref[$formname] as $index => $elem) {
                    if ($find_in == "ELEMENTS" && !is_object($elem)) {
                        continue;
                    }
                    if ($find_in == "ELEMENTS") {
                        self::log(
                            sprintf("current element [%s]", $elem->attr["id"]),
                            7
                        );
                        if (
                            (isset($elem->attr["id"]) &&
                                $elem->attr["id"] == $name) ||
                            (isset($elem->attr["name"]) &&
                                $elem->attr["name"] == $name)
                        ) {
                            self::log(
                                sprintf("found at index [%d]", $index),
                                7
                            );
                        ...
                        if (isset($elem["name"]) && $elem["name"] == $name) {
                            self::log(
                                sprintf("current element [%s]", $elem["name"]),
                                7
                            );
                            self::log(
                                sprintf("found at index [%d]", $index),
            ...
            if (isset($array_ref[$formname]) && count($array_ref[$formname])) {
                foreach ($array_ref[$formname] as $index => $elem) {
                    ...
                    if ($find_in == "ELEMENTS") {
                        ...
                        if (isset($elem["type"]) && $elem["type"] == $type) {
                            self::log(
                                sprintf("current element [%s]", $elem["name"]),
                                7
                            );
        ...
        public static function isValidated()
        {
            ...
            $formname = self::current();
            self::log("< isValidated()", 7);
            return isset(self::$FORMS[$formname]["__is_valid"]) ? true : false;
        ...
        public static function validateForm($get_empty = false)
        {
            ...
            if (wbFormsProtect::checkToken(self::$globals["token"])) {
                ...
                foreach (array_keys($ref) as $name) {
                    ...
                    if (
                        !substr_compare(
                            ...
                            self::$globals["honeypot_prefix"],
                            0,
                            strlen(self::$globals["honeypot_prefix"])
                        ) &&
                        $ref[$name] != ""
                    ) {
                        wbForms::setError(
                            self::t(
                                "You filled a honeypot field. Are you sure you are a human?"
                            )
                        );
                ...
                self::log(
                    sprintf("checking [%d] elements", count($elements)),
                    7
                );
                ...
                foreach ($elements as $elem) {
                    ...
                    if (
                        isset($elem["type"]) &&
                        array_key_exists($elem["type"], self::$nodata)
                    ) {
                        continue;
                    }
                    if (!isset($elem["name"])) {
                    ...
                    if (
                        ...
                        self::$ERRORS[$elem["name"]] = isset($elem["missing"])
                            ? self::t($elem["missing"])
                            : self::t("Please insert a value for this field");
                        self::log(
                            sprintf(
                                "no data for required field [%s]",
                                $elem["name"]
                            ),
                            7
                        );
                    ...
                    if (
                        ...
                        if (is_scalar($elem["allow"])) {
                            ...
                            $method = "check_" . $type;
                            if (
                                method_exists("\wblib\wbFormsProtect", $method)
                            ) {
                                if (
                                    ($value = wbFormsProtect::$method(
                                        $ref[$elem["name"]],
                                        $opt
                                    )) === false
                                ) {
                                    self::$ERRORS[$elem["name"]] = isset(
                                        $elem["invalid"]
                                    )
                                        ? self::t($elem["invalid"])
                                        : self::t(
                                    ...
                                    self::log(
                                        sprintf(
                                            "invalid data for field [%s], allowed [%s]",
                                            $elem["name"],
                                            $elem["allow"]
                                        ),
                                        7
                                ...
                                self::log(
                                    sprintf(
                                        "Invalid check method [%s]! (element [%s])",
                                        $method,
                                        $elem["name"]
                                    ),
                                    7
    ...
    class wbFormsElement extends wbFormsBase
    {
        ...
        protected function __construct($options = [])
        {
            ...
            foreach ($options as $attr => $val) {
                if (
                    strlen($attr) > 5 &&
                    substr_compare($attr, "data-", 0, 5) == 0
                ) {
                    $this->data[] = $attr . '="' . $val . '"';
                }
            }
            ...
            self::log(
                sprintf(
                    "element of type [%s] initialized with attr",
                    get_class($this)
                ),
                7
            );
        ...
        public function checkAttr()
        {
            ...
            if (
                isset($this->attr["required"]) &&
                $this->attr["required"] !== false
            ) {
                #if(!isset($this->attr['is_group']) || !$this->attr['is_group'])
                $this->attr["is_required"] = sprintf(
                    self::$globals["required_span"],
                    self::t("This item is required")
                );
                $this->attr["required"] = "required"; // valid XHTML
                $this->attr["aria-required"] = "true"; // WAI-ARIA
            } else {
                if (
                    !isset($this->attr["is_group"]) ||
                    !$this->attr["is_group"]
                ) {
    ...
    {
        protected static $tpl = '<div%class%%style%><span style="float:left;margin-right:.3em;" class="ui-icon ui-icon-info"></span>%label%</div>';
    ...
    {
        protected static $tpl = "<legend%id%%class%%style%>%label%</legend>\n";
        ...
        public function render()
        {
            return wbFormsElementFieldset::get()->open() . $this->replaceAttr();
        } // end function render()
    ...
    {
        protected static $tpl = "<label%for%%class%%style%>%label%</label>";
...
*/
    ...
    class wbFormsElementSelect extends wbFormsElement
    {
        ...
        public function render()
        {
            ...
            $options = [];
            $isIndexed =
                array_values($this->attr["options"]) === $this->attr["options"];
            $sel = [];
    ...
    {
        protected static $tpl = "%is_required%<input%type%%name%%id%%class%%style%%title%%value%%required%%aria-required%%tabindex%%accesskey%%disabled%%readonly%%onblur%%onchange%%onclick%%onfocus%%onselect% />%label%";
        public function init()
        {
    ...
    class wbFormsElementRadiogroup extends wbFormsElement
    {
        ...
        public function render()
        {
            ...
            foreach ($this->attr["options"] as $value => $key) {
                if ($isIndexed) {
                    // value is the index and should be ignored, so key is the value
                    $value = $key;
                }
                $checked = null;
                $class = null;
                ...
                if (
                    isset($this->attr["checked"]) &&
                    $this->attr["checked"] == $value
                ) {
                    $checked = "checked";
                }
                if (
                    is_array($this->attr["checked"]) &&
                    is_scalar($value) &&
                    isset($this->attr["checked"][$value])
                ) {
                    $checked = "checked";
                }
                if (
                    is_array($this->attr["checked"]) &&
                    in_array($value, $this->attr["checked"])
                ) {
                    $checked = "checked";
                }
    ...
    class wbFormsElementHoneypot extends wbFormsElement
    {
        ...
        {
            $this->checkAttr();
            $this->attr = array_merge($this->attr, [
                "type" => "text",
    ...
    class wbFormsJQuery extends wbFormsBase
    {
        ...
        protected static $globals = [
            ...
            "CDNs" => [
                ...
                "jqueryuithemes" =>
                    "//ajax.googleapis.com/ajax/libs/jqueryui/1.11.4/themes/%s/jquery-ui.css",
        ...
        public static function init()
        {
            ...
            wbFormsJQuery::$tpl = '<script type="text/javascript">%s</script>';
            self::log("< jQuery::init()", 7);
        } // end function init()
        ...
        public static function addComponent($name)
        {
            ...
            if (!isset(self::$ui_components[$name])) {
                ...
                array_push(self::$ui_components, $name);
            }
            self::log("< jQuery::addComponent()", 7);
        ...
        public static function attach($id, $comp, $opt = null)
        {
            ...
            array_push(self::$attach_comp[$id], [$comp => $opt]);
            self::log("< jQuery::attach()", 7);
        } // end function attach()
        ...
        public static function getHeaders()
        {
            ...
            if (count(wbForms::$CSS)) {
                $output .= implode(
                    ...
                    array_map(function ($url) {
                        return sprintf(wbFormsJQuery::$css_tpl, $url);
            ...
            if (count(wbForms::$INLINECSS)) {
                $output .= implode(
                    ...
                    array_map(function ($css) {
                        return sprintf(wbFormsJQuery::$inline_css_tpl, $css);
            ...
            $output .= implode(
                ...
                array_map(function ($script) {
                    return sprintf(wbFormsJQuery::$script_tpl, $script, null);
            ...
            $output .= implode(
                "\n",
                array_map(function ($script) {
                    return sprintf(wbFormsJQuery::$script_tpl, $script);
        ...
        public static function render()
        {
            ...
            if (!self::$global_code_sent) {
                ...
                if (self::$globals["disable_tooltips"] === true) {
                    $code1 .=
                        "            var wbforms_disable_tooltips = true;" .
                        "\n";
                }
        ...
        <script src=\"" .
                    WBLIB_URL .
                    "/3rdparty/js/requirejs/require.js\" data-main=\"" .
                    WBLIB_URL .
                    "/3rdparty/js/loader\"></script>
";
            }
            self::log("< jQuery::render()", 7);
    ...
    class wbFormsProtect extends wbFormsBase
    {
        ...
        {
            if (
                is_callable(["\wblib\wbValidateValidate", "as_string"]) === true
            ) {
        ...
        public static function check_int($var, $opt = "")
        {
            if (substr_count($var, ":")) {
                ...
                if ($min && is_numeric((int) $min)) {
                    $opt["options"]["min_range"] = $min;
                }
                if ($max && is_numeric((int) $max)) {
        ...
        public static function checkToken($dynamic = null)
        {
            ...
            if (count($parts) !== 3) {
                self::log(
                    sprintf(
                        "Invalid token - parts [%s], should be [3]",
                        count($parts)
                    ),
                    1
                );
            ...
            $tokentime = self::$config["token_lifetime"]
                ? self::$config["token_lifetime"]
                : self::$token_lifetime;
            // secret time should not extend one day and not drop below 1 hour
            if (
                !is_numeric($tokentime) ||
                $tokentime > 86400 ||
                $tokentime < 36000
            ) {
                self::log(
                    sprintf(
                        "Invalid token lifetime [%s] given; using the default (86400 = 1 day)",
                        $tokentime
                    ),
                    5
                );
        ...
        public static function createToken($dynamic = null, $force = false)
        {
            ...
            $hash = sha1(
                self::createSecret($dynamic) . "-" . $dynamic . "-" . $token
            );
        ...
        private static function createSecret($dynamic)
        {
            ...
            if (
                ...
                self::log(
                    "createSecret() - Invalid token lifetime given; using the default (86400 = 1 day)",
                    4
                );
                $tokentime = 86400;
            }
            ...
            $serverdata = isset($_SERVER["SERVER_SIGNATURE"])
                ? $_SERVER["SERVER_SIGNATURE"]
                : "qN";
            $serverdata .= isset($_SERVER["SERVER_SOFTWARE"])
                ? $_SERVER["SERVER_SOFTWARE"]
                : "qp";
            $serverdata .= isset($_SERVER["SERVER_NAME"])
                ? $_SERVER["SERVER_NAME"]
                : "q8";
            $serverdata .= isset($_SERVER["SERVER_ADDR"])
                ? $_SERVER["SERVER_ADDR"]
                : "q&";
            $serverdata .= isset($_SERVER["SERVER_PORT"])
                ? $_SERVER["SERVER_PORT"]
                : "q1";
            $serverdata .= isset($_SERVER["SERVER_ADMIN"])
            ...
            $browser .= isset($_SERVER["HTTP_ACCEPT_CHARSET"])
                ? $_SERVER["HTTP_ACCEPT_CHARSET"]
                : "xB";

            self::log(
                sprintf(
                    "seed [%s] serverdata [%s] browserdata [%s]",
                    $Seed,
                    $serverdata,
                    $browser
                ),
