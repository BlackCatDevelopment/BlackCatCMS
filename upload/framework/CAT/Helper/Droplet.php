...
if (!class_exists("CAT_Helper_Droplet")) {
    ...
    if (
        file_exists(
            CAT_PATH . "/modules/" . SEARCH_LIBRARY . "/search.droplets.php"
        )
    ) {
    ...
    class CAT_Helper_Droplet extends CAT_Object
    {
         ...
         * @return string
         **/
        public static function getDropletsForHeader(
            $page_id,
            $open_graph = false
        ) {
            $title = null;
                    ...
                    if (
                        ...
                        if (
                            ...
                            include CAT_PATH .
                                ...
                                "/droplet.extension.php";
                            $user_func =
                                $droplet["drop_module_dir"] . "_droplet_header";
                            if (function_exists($user_func)) {
                                ...
                                if (is_array($header)) {
                                    ...
                                    if (
                                        isset($header["keywords"]) &&
                                        !empty($header["keywords"])
                                    ) {
                        ...
                        self::unregister_droplet_header(
                            $droplet["drop_droplet_name"],
                            $page_id
                        );
                    }
            ...
            if (count($droplets)) {
                foreach ($droplets as $droplet) {
                    ...
                    if (self::droplet_exists($droplet["drop_droplet_name"])) {
                        ...
                        self::unregister_droplet(
                            $droplet["drop_droplet_name"],
                            $droplet["drop_type"],
                            $page_id
                        );
                        // also unregister droplet search
                        self::unregister_droplet_search(
                            $droplet["drop_droplet_name"],
                            $page_id
                        );
                    }
            ...
            if (file_exists(CAT_PATH . "/modules/droplets/config.json")) {
                $config = json_decode(
                    file_get_contents(
                        CAT_PATH . "/modules/droplets/config.json"
                    ),
                    true
                );
            }
            ...
            if (empty($image)) {
                if (
                    false !==
                    ($test = self::getFirstImageFromContent(
                        $page_id,
                        $exec_droplets
                    ))
                ) {
            ...
            if (!empty($image)) {
                $ext = pathinfo(
                    CAT_PATH . substr($image, strlen(CAT_URL)),
                    PATHINFO_EXTENSION
                );
                if (
                    !in_array(strtolower($ext), ["png", "gif", "jpg", "jpeg"])
                ) {
                    $image = "";
                }
            }
        ...
        public static function droplet_exists($droplet_name)
        {
            ...
            $params = ["name" => $droplet_name];
            $result = CAT_Helper_DB::getInstance()->query($SQL, $params);
         ...
         * @return boolean
         */
        public static function is_registered_droplet(
            $droplet_name,
            $register_type,
            $page_id
        ) {
            $droplet_name = self::clear_droplet_name($droplet_name);
            $SQL =
                "SELECT `drop_page_id` FROM `:prefix:mod_droplets_extension` " .
            ...
            $check = CAT_Helper_DB::getInstance()
                ->query($SQL, $params)
                ->fetchColumn();
            if (CAT_Helper_DB::getInstance()->isError() || !$check) {
                return false;
            }
         ...
         * @return BOOL
         */
        public static function is_registered_droplet_search(
            $droplet_name,
            $page_id
        ) {
            return self::is_registered_droplet(
                $droplet_name,
                "search",
                $page_id
            );
         ...
         * @return BOOL
         */
        public static function is_registered_droplet_header(
            $droplet_name,
            $page_id
        ) {
            return self::is_registered_droplet(
         ...
         * @return BOOL
         */
        public static function is_registered_droplet_css(
            $droplet_name,
            $page_id
        ) {
         ...
         * @return boolean
         */
        public static function register_droplet(
            $droplet_name,
            $page_id,
            $module_directory,
            $register_type,
            ...
            $module_directory = CAT_Helper_Directory::sanitizePath(
                $module_directory
            );
            // insert
            $SQL =
                "INSERT INTO `:prefix:mod_droplets_extension` " .
         ...
         * @return BOOL
         */
        public static function register_droplet_search(
            $droplet_name,
            $page_id,
            $module_directory
        ) {
            return self::register_droplet(
                $droplet_name,
                $page_id,
                $module_directory,
                "search"
         ...
         * @return BOOL
         */
        public static function register_droplet_header(
            $droplet_name,
            $page_id,
            $module_directory
        ) {
         ...
         * @return BOOL
         */
        public static function register_droplet_css(
            $droplet_name,
            $page_id,
            $module_directory,
            $file
         ...
         * @return BOOL
         */
        public static function register_droplet_js(
            $droplet_name,
            $page_id,
            $module_directory,
            $file
         ...
         * @return BOOL
         */
        public static function unregister_droplet(
            $droplet_name,
            $register_type,
            $page_id
        ) {
            ...
            // delete the record
            $SQL =
                "DELETE FROM `:prefix:mod_droplets_extension` WHERE `drop_droplet_name`=:name " .
         ...
         * @return BOOL
         */
        public static function unregister_droplet_search(
            $droplet_name,
            $page_id
        ) {
         ...
         * @return BOOL
         */
        public static function unregister_droplet_header(
            $droplet_name,
            $page_id
        ) {
        ...
        {
            return self::unregister_droplet(
                $droplet_name,
                "javascript",
                $page_id
            );
        ...
        public static function check_syntax($code)
        {
            ...
            if ($check_result === false) {
                if ($braces) {
                    ...
                    false !== strpos($code, CR) &&
                        ($code = strtr(str_replace(CRLF, LF, $code), CR, LF));
        ...
        {
            $query = CAT_Helper_DB::getInstance()->query(
        ...
        {
            $stmt = CAT_Helper_DB::getInstance()->query(
        ...
        public static function getDroplets($with_code = false)
        {
            ...
            $rows = [];
            $fields =
                "t1.id, `name`, `description`, `active`, `comments`, `view_groups`, `edit_groups`";
                ...
                while ($droplet = $query->fetch()) {
                    ...
                    if (
                        ...
                        if ($droplet["edit_groups"]) {
                            if (
                                CAT_Users::get_user_id() != 1 &&
                                !is_in_array($droplet["edit_groups"], $groups)
                            ) {
                                continue;
                            } else {
                    ...
                    if ($with_code) {
                        $droplet["valid_code"] = self::check_syntax(
                            $droplet["code"]
                        );
                    }
         ...
         */
        public static function getFirstImageFromContent(
            $page_id,
            $exec_droplets = true
        ) {
            $self = self::getInstance();
            ...
            if (is_array($section) && count($section)) {
                ...
                $params = ["id" => $section[0]["section_id"]];
                $result = $self
                    ->db()
                    ->query($SQL, $params)
                    ->fetchColumn();
                ...
                if (is_string($result)) {
                    $__CAT_Helper_Droplets_content = self::unsanitizeText(
                        $result
                    );
                }
            }
                ...
                if (
                    ...
                    file_exists(
                        CAT_Helper_Directory::sanitizePath(
                            CAT_PATH . "/modules/droplets/droplets.php"
                        )
                    )
                ) {
                    ...
                    ob_start();
                    include_once CAT_Helper_Directory::sanitizePath(
                        CAT_PATH . "/modules/droplets/droplets.php"
                    );
                    if (function_exists("evalDroplets")) {
                        try {
                            $__CAT_Helper_Droplets_content = evalDroplets(
                                $__CAT_Helper_Droplets_content
                            );
                        } catch (Exception $e) {
                            trigger_error(
        ...
        public static function deleteDroplet($id)
        {
            ...
            $self
                ->db()
                ->query("DELETE FROM `:prefix:mod_droplets` WHERE id = :id", [
                    "id" => $id,
                ]);
            if ($self->db()->isError()) {
                $error = $self
        ...
        public static function insertDroplet($values)
        {
            $self = self::getInstance();
            $query =
                "INSERT INTO `:prefix:mod_droplets` VALUES " .
                "(NULL, :name, :code, :description, :time, :userid, :active, 1, 1, :wysiwyg, :comment )";

            // some defaults
            foreach (["active", "wysiwyg"] as $key) {
                if (!isset($values[$key])) {
                    $values[$key] = 1;
                }
            }
        ...
        public static function process(
            ...
            while (
                self::evaluate($__CAT_Helper_Droplets_content) === true &&
                $max_loops > 0
            ) {
                $max_loops--;
            }
        ...
        public static function installDroplet($temp_file)
        {
            ...
            // extract file
            $list = CAT_Helper_Zip::getInstance($temp_file)
                ->config("Path", $temp_unzip)
                ->extract();

            // get .php files
            $files = CAT_Helper_Directory::getPHPFiles(
                $temp_unzip,
                $temp_unzip . "/"
            );
            ...
            foreach ($files as $file) {
                if (
                    pathinfo($file, PATHINFO_FILENAME) !== "index" &&
                    ...
                    if (!$description && !$usage) {
                        // invalid file
                        $errors[$file] = CAT_Helper_Directory::getInstance()
                            ->lang()
                            ->translate(
                                "No valid Droplet file (missing description and/or usage instructions)"
                            );
                        continue;
                    }
                    ...
                    // execute
                    $q =
                        "$stmt INTO `:prefix:mod_droplets` SET " .
                        ($id ? "id=" . $id . ", " : "") .
        ...
        {
            $SQL =
                "SELECT `drop_module_dir`,`drop_droplet_name` " .
                "FROM `:prefix:mod_droplets_extension` " .
                "WHERE `drop_type`=:type AND `drop_page_id`=:id LIMIT 1";
            $params = ["type" => $type, "id" => $page_id];
            if (
                null ==
                ($query = CAT_Helper_DB::getInstance()->query($SQL, $params))
            ) {
                return false;
            }
            if ($query->rowCount() > 0) {
        ...
        {
            $SQL =
                "SELECT * FROM `:prefix:mod_droplets_extension` " .
         ...
         * @return eval result
         **/
        private static function do_eval(
            $_x_codedata,
            $_x_varlist,
            &$__CAT_Helper_Droplets_content
        ) {
            ...
            self::getInstance()
                ->log()
                ->LogDebug("evaluating: " . $_x_codedata);
        ...
        private static function evaluate(&$__CAT_Helper_Droplets_content)
        {
            ...
            if (
                ...
                foreach ($found_droplets[1] as $droplet) {
                    if (
                        ...
                        if ($request_string != "") {
                            ...
                            $request_string = html_entity_decode(
                                $request_string,
                                ENT_COMPAT,
                                DEFAULT_CHARSET
                            );
                        ...
                        if (!is_null($codedata)) {
                            ...
                            if ($newvalue == "" && $newvalue !== true) {
                                $self
                                    ->log()
                                    ->LogDebug(
                                        "newvalue is empty and not true (this is an error!)"
                                    );
                            ...
                            if ($newvalue === true) {
                                $self
                                    ->log()
                                    ->LogDebug(
                                        "newvalue is true, set to empty string"
                                    );
                                $newvalue = "";
                            }
                            $self
                                ->log()
                                ->LogDebug(
                                    "newvalue before removing styles:",
                                    $newvalue
                                );
                            ...
                            $self
                                ->log()
                                ->LogDebug(
                                    "newvalue after removing styles:",
                                    $newvalue
                                );
                        } else {
                            ...
                            if ($self->_config["loglevel"] == 7) {
                                $newvalue =
                                    '<span class="mod_droplets_err">No such droplet: ' .
                                    $droplet .
                                    "</span>";
                            } else {
                                $newvalue = null;
                ...
                $__CAT_Helper_Droplets_content = str_replace(
                    $droplet_tags,
                    $droplet_replacements,
                    $__CAT_Helper_Droplets_content
                );
