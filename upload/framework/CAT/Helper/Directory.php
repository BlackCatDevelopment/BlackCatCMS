...
if (!class_exists("CAT_Helper_Directory", false)) {
  class CAT_Helper_Directory extends CAT_Object
  {
    ...
    public static function findDirectories($pattern, $dir, $remove_dir = false)
    {
      ...
      foreach ($list as $entry) {
        ...
        if (preg_match("~^$pattern$~i", pathinfo($entry, PATHINFO_BASENAME))) {
          $dirs[] = $remove_dir ? str_ireplace($dir ?? "", "", $entry) : $entry;
        }
      }
      return $dirs;
    ...
    {
      return self::scanDirectory($dir, true, true, $remove_prefix, ["php"]);
    ...
    public static function sanitizePath($path)
    {
      ...
      foreach (explode("/", preg_replace("~/+~", "/", $path)) as $part) {
        if ($part === ".." || $part == "") {
          ...
          $part =
            self::$is_win && mb_detect_encoding($part, "UTF-8", true)
              ? utf8_decode($part)
              : $part;
          $parts[] = $part;
        }
        ...
        while (false !== ($file = $dh->read())) {
          ...
          if (!($file == "." || $file == "..")) {
            ...
            if (is_dir($dir . "/" . $file)) {
              ...
              if (
                !count($suffixes) ||
                in_array(pathinfo($file, PATHINFO_EXTENSION), $suffixes)
              ) {
