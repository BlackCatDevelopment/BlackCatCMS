...
if (!class_exists("CAT_Helper_DateTime")) {
    ...
    class CAT_Helper_DateTime extends CAT_Object
    {
        ...
        {
            $tz = CAT_Helper_Validate::getInstance()->fromSession(
                "TIMEZONE_STRING"
            );
        ...
        public static function getTimeFormats()
        {
            ...
            if (isset($user_time) and $user_time == true) {
                $TIME_FORMATS["system_default"] =
                    date(DEFAULT_TIME_FORMAT, $actual_time) .
                    " (System Default)";
                $TIME_FORMATS = array_reverse($TIME_FORMATS, true);
            }
            if (
                isset($language_time) &&
                !array_key_exists($language_time, $TIME_FORMATS)
            ) {
                $TIME_FORMATS[$language_time] = date(
                    $language_time,
                    $actual_time
                );
            }
        ...
        public static function getDateFormats()
        {
            ...
            $DATE_FORMATS = [
                "%A,|%e|%B,|%Y" => $toUtf8(self::strftime($long, $actual_time)),
                "%e|%B,|%Y" =>
                    $toUtf8(
                        self::strftime(
                            str_replace(" #O#", $ord, $ord_long),
                            $actual_time
                        )
                    ) . " (jS F, Y)",
                "%d|%m|%Y" =>
                    $toUtf8(self::strftime("%d %m %Y", $actual_time)) .
                    " (d M Y)",
                "%b|%d|%Y" =>
                    $toUtf8(self::strftime("%b %d %Y", $actual_time)) .
                    " (M d Y)",
                "%a|%b|%d,|%Y" =>
                    $toUtf8(self::strftime("%a %b %d, %Y", $actual_time)) .
                    " (D M d, Y)",
                "%d-%m-%Y" =>
                    $toUtf8(self::strftime("%d-%m-%Y", $actual_time)) .
                    " (D-M-Y)",
                "%m-%d-%Y" =>
                    $toUtf8(self::strftime("%m-%d-%Y", $actual_time)) .
                    " (M-D-Y)",
                "%d.%m.%Y" =>
                    $toUtf8(self::strftime("%d.%m.%Y", $actual_time)) .
                    " (D.M.Y)",
                ...
                "%a, %d %b %Y %H:%M:%S %z" =>
                    $toUtf8(
                        self::strftime("%a, %d %b %Y %H:%M:%S %z", $actual_time)
                    ) . " (r)",
                "%A,|%d.|%B|%Y" => $toUtf8(
                    self::strftime("%A, %d. %B %Y", $actual_time)
                ), // German date
            ];
            if (isset($user_time) && $user_time == true) {
                $DATE_FORMATS["system_default"] =
                    date(DEFAULT_DATE_FORMAT, $actual_time) .
                    " (System Default)";
                $DATE_FORMATS = array_reverse($DATE_FORMATS, true);
            }
            if (
                isset($language_date_long) &&
                !array_key_exists($language_date_long, $DATE_FORMATS)
            ) {
                $DATE_FORMATS[$language_date_long] = date(
                    $language_date_long,
                    $actual_time
                );
            }
            if (
                isset($language_date_short) &&
                !array_key_exists($language_date_short, $DATE_FORMATS)
            ) {
                $DATE_FORMATS[$language_date_short] = date(
                    $language_date_short,
                    $actual_time
                );
            }
        ...
        public static function getDefaultTimeFormat()
        {
            ...
            if (isset($_SESSION["CAT_TIME_FORMAT"])) {
                return $_SESSION["CAT_TIME_FORMAT"];
            }
            // default format
            if (defined("CAT_DEFAULT_TIME_FORMAT")) {
                return CAT_DEFAULT_TIME_FORMAT;
            }
            // language file
            if (isset($language_time)) {
                return $language_time;
            }
            // global default
            return "%H:%M";
        ...
        public static function getDefaultDateFormatShort()
        {
            ...
            if (defined("CAT_DEFAULT_DATE_FORMAT_SHORT")) {
                return CAT_DEFAULT_DATE_FORMAT_SHORT;
            }
            // language file
            if (isset($language_date_short)) {
                return $language_date_short;
            }
            // default format
            if (defined("CAT_DEFAULT_DATE_FORMAT")) {
                return CAT_DEFAULT_DATE_FORMAT;
            }
            // global default
            return "%d-%m-%Y";
        ...
        public static function getDefaultDateFormatLong()
        {
            ...
            $format = null;
            if (defined("CAT_DEFAULT_DATE_FORMAT")) {
                $format = CAT_DEFAULT_DATE_FORMAT;
            } elseif (isset($language_date_long)) {
                $format = $language_date_long;
            } else {
                $format = "%x";
            }
            $format .= " " . self::getDefaultTimeFormat();
        ...
        public static function strftime(
            ...
            $intl_formatter = function (
                \DateTimeInterface $timestamp,
                string $format
            ) use ($intl_formats, $locale) {
            ...
            $translation_table = [
                ...
                "%U" => function ($timestamp) {
                    ...
                    $day = new \DateTime(
                        sprintf("%d-01 Sunday", $timestamp->format("Y"))
                    );
                    return sprintf(
                        "%02u",
                        1 + ($timestamp->format("z") - $day->format("z")) / 7
                    );
                },
                "%V" => "W",
                "%W" => function ($timestamp) {
                    ...
                    $day = new \DateTime(
                        sprintf("%d-01 Monday", $timestamp->format("Y"))
                    );
                    return sprintf(
                        "%02u",
                        1 + ($timestamp->format("z") - $day->format("z")) / 7
            ...
            $out = preg_replace_callback(
                ...
                function ($match) use ($translation_table, $timestamp) {
                    ...
                    if (!isset($translation_table[$match[1]])) {
                        throw new \InvalidArgumentException(
                            sprintf(
                                'Format "%s" is unknown in time format',
                                $match[1]
                            )
                        );
                    }
