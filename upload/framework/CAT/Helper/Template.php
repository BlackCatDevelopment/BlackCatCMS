...
if (!class_exists("CAT_Helper_Template")) {
    ...
    {
        protected int $debuglevel = CAT_Helper_KLogger::CRIT;
        protected $_config = ["loglevel" => CAT_Helper_KLogger::CRIT];
        private static array $_drivers = [];
        private static ?string $_driver = null;
        protected static array $template_menus = [];
        ...
        public function __construct($compileDir = null, $cacheDir = null)
        {
            ...
            $this->workdir =
                isset($callstack[0]) &&
                isset($callstack[0]["file"]) &&
                pathinfo($callstack[0]["file"], PATHINFO_BASENAME) !==
                    "DriverDecorator.php"
        ...
        public static function getInstance($driver)
        {
            if (
                !(
                    strcasecmp(
                        substr($driver, strlen($driver) - strlen("driver")),
                        "driver"
                    ) === 0
                )
            ) {
            ...
            if (
                !file_exists(
                    dirname(__FILE__) . "/Template/" . $driver . ".php"
                )
            ) {
                $s = new self();
                $s->printFatalError(
                    $s->lang->translate(
                        "No such template driver: [" . $driver . "]"
                    )
                );
            }
            self::$_driver = $driver;
            if (
                !isset(self::$_drivers[$driver]) ||
                !is_object(self::$_drivers[$driver])
            ) {
                ...
                self::$_drivers[
                    $driver
                ] = new CAT_Helper_Template_DriverDecorator(new $driver());
                ...
                foreach (["TEXT", "HEADING", "MESSAGE"] as $global) {
         ...
         * @return
         **/
        public static function get_template_block_name(
            ...
            if (SECTION_BLOCKS != false) {
                ...
                return isset($block[$selected])
                    ? $block[$selected]
                    : $driver->lang()->translate("Main");
            }
         ...
         * @return void
         */
        public static function get_template_blocks(
            $template = DEFAULT_TEMPLATE,
            $selected = 1
        ) {
            global $page_id;
            ...
            if (SECTION_BLOCKS != false) {
                ...
                if (!isset($block[1]) || $block[1] == "") {
                    if (isset($blocks)) {
                        ...
                        if (!$variant) {
                            $variant =
                                defined("DEFAULT_TEMPLATE_VARIANT") &&
                ...
                foreach ($block as $number => $name) {
                    ...
                    if ($selected == $number || $selected == $name) {
                        self::getInstance(self::$_driver)->current_block = [
                            "name" => $name,
                            "id" => $number,
                        ];
                    }
         ...
         * @return void
         */
        public static function get_template_menus(
            $template = DEFAULT_TEMPLATE,
            $selected = 1
        ) {
        ...
        public static function getVariants($template = DEFAULT_TEMPLATE)
        {
            ...
            if (file_exists($info)) {
                ...
                include $info;
                if (
                    isset($template_variants) &&
                    is_array($template_variants) &&
                    count($template_variants)
                ) {
