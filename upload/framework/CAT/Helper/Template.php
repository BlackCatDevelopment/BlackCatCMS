...
if (!class_exists("CAT_Helper_Template")) {
  ...
  class CAT_Helper_Template extends CAT_Object
  {
    ...
    protected static $template_menus = [];
    ...
    public static function getInstance($driver)
    {
      ...
      if (!file_exists(dirname(__FILE__) . "/Template/" . $driver . ".php")) {
        $s = new self();
        $s->printFatalError(
          $s->lang->translate("No such template driver: [" . $driver . "]")
        );
      }
      self::$_driver = $driver;
      if (
        ...
        self::$_drivers[$driver] = new CAT_Helper_Template_DriverDecorator(
          new $driver()
        );
      ...
      if (SECTION_BLOCKS != false) {
        ...
        foreach ($block as $number => $name) {
          $driver->template_block[$number] = [
            ...
            "VALUE" => $number,
            "SELECTED" =>
              $selected == $number || $selected == $name ? true : false,
          ];
