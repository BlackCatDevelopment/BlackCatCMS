...
if (!class_exists("CAT_Helper_I18n", false)) {
    class CAT_Helper_I18n extends CAT_Object
    {
        ...
        {
            // PHP 8.x: Using null as an array offset is deprecated
            if ($lang === null) {
                $lang = "";
            }
            if (
                !isset(self::$instances[$lang]) ||
                !is_object(self::$instances[$lang])
            ) {
                self::$instances[$lang] = new self(["lang" => $lang]);
            }
        ...
        public function init($var = null)
        {
            ...
            while (
                ($caller && !isset($caller["file"])) ||
                (isset($caller["class"]) &&
                    $caller["class"] == "CAT_Helper_I18n" &&
            ...
            if ($caller) {
                if (
                    ...
                    $this->_config[
                        ...
                        dirname($caller["file"]) . $this->_config["langPath"]
                    );
                } elseif (
                    file_exists(
                        CAT_Helper_Directory::sanitizePath(
        ...
        public function setPath($path, $var = null)
        {
            if (file_exists($path)) {
                ...
                $this->printError(
                    "language file path does not exist: " . $path
                );
            }
         ...
         *
         **/
        public function translate($msg, $attr = [])
        {
         ...
         * @return
         **/
        public static function getUsedLangs(
            $guest_only = true,
            $langs_only = false
        ) {
            ...
            foreach ($pages as $id) {
                ...
                if ($langs_only) {
                    ...
                    $langs[$lang][] = [
                        "page_id" => $id,
                        "menu_title" => CAT_Helper_Page::properties(
                            $id,
                            "menu_title"
                        ),
                    ];
