...
if (!class_exists("CAT_Helper_Upload")) {
    ...
    class CAT_Helper_Upload extends CAT_Object
    {
        ...
        protected string $file_max_size_raw = "";
        protected int $file_max_size = 0;
        ...
        function __construct($file)
        {
            ...
            if (!is_array($file)) {
                if (empty($file)) {
                    ...
                    if ($this->uploaded) {
                        ...
                        if (is_array($extension) && sizeof($extension) > 0) {
                            $this->file_src_name_ext = strtolower(
                                $extension[1]
                            );
                            $this->file_src_name_body = substr(
                                $this->file_src_name,
                                0,
                                strlen($this->file_src_name) -
                                    strlen($this->file_src_name_ext) -
                                    1
                            );
                        } else {
                            $this->file_src_name_ext = "";
                        ...
                        $this->file_src_size = file_exists($file)
                            ? filesize($file)
                            : 0;
                    }
                ...
                if ($this->uploaded) {
                    $this->file_src_error = isset($file["error"][0])
                        ? trim($file["error"][0])
                        : (isset($file["error"])
                            ? trim($file["error"])
                            : null);
                ...
                if ($this->uploaded) {
                    ...
                    if ($this->file_src_name == "") {
                        $this->uploaded = false;
                        $this->error =
                            "File upload error. Unable to determine source file name.";
                        $this->log()->logError(
            ...
            if ($this->uploaded) {
                ...
                $this->log()->logDebug(
                    "file_src_pathname  : " . $this->file_src_pathname
                );
                $this->log()->logDebug(
                    "file_src_mime      : " . $this->file_src_mime
                );
                $this->log()->logDebug(
                    "file_src_size      : " .
                        $this->file_src_size .
                ...
                $this->log()->logDebug(
                    "file_src_error     : " . $this->file_src_error
                );
            }
        ...
        function getsize($size)
        {
            if (preg_match('~^\d+$~', $size)) {
                $this->log()->logDebug(
                    sprintf(
                        "getsize() - size %s is numeric, nothing to convert",
                        $size
                    )
                );
                return (int) $size;
        ...
        function process($server_path = null)
        {
            ...
            if ($this->processed) {
                ...
                if ($this->no_script) {
                    ...
                    if (
                        $this->file_force_extension &&
                        empty($file_src_name_ext)
                    ) {
                        if (
                            $key = array_search(
                                $this->file_src_mime,
                            ...
                            $file_src_name =
                                $file_src_name_body . "." . $file_src_name_ext;
                            $this->log()->logDebug(
                                "file renamed as " .
                                    $file_src_name_body .
                                    "." .
                    ...
                    if (
                        (((substr($this->file_src_mime, 0, 5) == "text/" &&
                            $this->file_src_mime != "text/rtf") ||
                            strpos($this->file_src_mime, "javascript") !==
                                false) &&
                            substr($file_src_name, -4) != ".txt") ||
                        preg_match(
                            '/\.(php|pl|py|cgi|asp|js)$/i',
                            $this->file_src_name
                        ) ||
                        ($this->file_force_extension &&
                            empty($file_src_name_ext))
                ...
                if ($this->mime_check && empty($this->file_src_mime)) {
                    ...
                    strpos($this->file_src_mime, "/") !== false
                ) {
                    list($m1, $m2) = explode("/", $this->file_src_mime);
                    $this->log()->logDebug(
                        sprintf("checking mime type [%s]", $this->file_src_mime)
                    );
                    $allowed = false;
                    ...
                    foreach ($this->allowed as $k => $v) {
                        ...
                        $this->log()->logDebug(
                            sprintf(
                                "checking allowed %s/%s against %s/%s",
                                $v1,
                                $v2,
                                $m1,
                                $m2
                    ...
                    foreach ($this->forbidden as $k => $v) {
                        ...
                        $this->log()->logDebug(
                            sprintf(
                                "checking forbidden %s/%s against %s/%s",
                                $v1,
                                $v2,
                                $m1,
                                $m2
                    ...
                    if (!$allowed) {
                        $this->processed = false;
                        $this->error =
                            "Incorrect type of file. Mime type [" .
                            $this->file_src_mime .
                            "] is forbidden";
                    } else {
                        $this->log()->logDebug(
            ...
            if ($this->processed) {
                ...
                if (!is_null($this->file_name_body_pre)) {
                    ...
                    $this->file_dst_name_body =
                        $this->file_name_body_pre . $this->file_dst_name_body;
                    $this->log()->logDebug(
                ...
                if ($this->file_safe_name) {
                    ...
                    $this->file_dst_name_body = preg_replace(
                        ["/\s/", "/\.[\.]+/", "/[^\w_\.\-]/"],
                        ["_", ".", ""],
                        $this->file_dst_name_body
                    );
                ...
                if (!$return_mode) {
                    if (!$this->file_auto_rename) {
                        ...
                        $this->file_dst_pathname =
                            $this->file_dst_path . $this->file_dst_name;
                    } else {
                        ...
                        if ($file_src_name_ext != $this->file_src_name_ext) {
                            if (
                                substr(
                                    $this->file_dst_name_body,
                                    -1 - strlen($this->file_src_name_ext)
                                ) ==
                                "." . $this->file_src_name_ext
                            ) {
                                $body = substr(
                                    $this->file_dst_name_body,
                                    0,
                                    strlen($this->file_dst_name_body) -
                                        1 -
                                        strlen($this->file_src_name_ext)
                                );
            ...
            if ($this->processed) {
                ...
                if (!$return_mode) {
                    if (
                        $this->processed &&
                        !file_exists($this->file_dst_path)
                    ) {
                    ...
                    if (
                        ...
                        if ($this->dir_auto_chmod) {
                            ...
                            if (
                                !@chmod($this->file_dst_path, $this->dir_chmod)
                            ) {
                                $this->log()->logDebug("--> failed");
                                $this->processed = false;
                                $this->error =
        ...
        public function getMimeType()
        {
            ...
            if (
                file_exists(CAT_PATH . "/modules/lib_getid3/getid3/getid3.php")
            ) {
                $this->log()->logDebug(
                    "- Checking MIME type with getID3 library"
                );
            ...
            elseif ($this->mime_fileinfo) {
                $this->log()->logDebug(
                    "- Checking MIME type with PECL extension"
                );
        ...
        public function getID3Mime()
        {
            ...
            if (!file_exists($filename)) {
                $this->error =
                    'File does not exist: "' . htmlentities($filename);
                return false;
            } elseif (!is_readable($filename)) {
                $this->error =
                    'File is not readable: "' . htmlentities($filename);
                return false;
            }
            ...
            $this->log()->logDebug(
                "MIME type detected as [" . $mime . "] by getID3 library"
            );
            return $mime;
        ...
        public function getPECLMime()
        {
            ...
            elseif (@class_exists("finfo")) {
                ...
                if ($f) {
                    ...
                    $this->log()->logDebug(
                        "Fileinfo PECL extension failed (finfo)"
                    );
                }
            } else {
                ...
                if (function_exists("exec")) {
                    if (
                        strlen(
                            $mime = @exec(
                                "file -bi " .
                                    escapeshellarg($this->file_src_pathname)
                            )
                        ) != 0
                    ) {
                        $mime = trim($mime);
        ...
        public function getMagicMime()
        {
            ...
            if (function_exists("mime_content_type")) {
                ...
                $this->log()->logDebug(
                    "MIME type detected as " . $mime . " by mime_content_type()"
                );
                if (preg_match("/^([\.-\w]+)\/([\.-\w]+)(.*)$/i", $mime)) {
                    $mime = preg_replace(
                        "/^([\.-\w]+)\/([\.-\w]+)(.*)$/i",
