    ...
    class CAT_Helper_Template_DriverDecorator extends CAT_Helper_Template
    {
        ...
        public function __call($method, $args)
        {
            ...
            switch (count($args)) {
                ...
                default:
                    return call_user_func_array([$this->te, $method], $args);
            }
        ...
        public function setPath($path, $context = "frontend")
        {
            ...
            $this->last = null;
            $this->log()->logDebug(
                sprintf("context [%s] path [%s]", $context, $path)
            );
            if (file_exists($path)) {
                ...
                $this->log()->logWarn(
                    "unable to set template path: does not exist!",
                    $path
                );
                return false;
            }
        ...
        public function setFallbackPath($path, $context = "frontend")
        {
            $path = CAT_Helper_Directory::sanitizePath($path);
            $this->log()->logDebug(
                sprintf("context [%s] fallback path [%s]", $context, $path)
            );
            if (file_exists($path)) {
                $this->te->paths[$context . "_fallback"] = $path;
                return true;
                ...
                $this->log()->logWarn(
                    "unable to set fallback template path: does not exist!",
                    $path
                );
                return false;
            }
        ...
        public function findTemplate($_tpl)
        {
            ...
            $this->dirh->setSuffixFilter(["tpl", "htt", "lte"]);
            ...
            foreach ($paths as $dir) {
                ...
                if ($file) {
                    $this->seen[
                        $this->te->paths["current"] . "/" . $_tpl
                    ] = $file;
                    return $file;
                }
            }
            $this->log()->logCrit(
                "The template [$_tpl] does not exist in one of the possible template paths!",
                $paths
            );
