...
if (!class_exists("CAT_Helper_Addons")) {
    ...
    class CAT_Helper_Addons extends CAT_Object
    {
        ...
        private static $info_vars_mandatory = [
            "module" => [
                "module_author",
                "module_name",
                "module_directory",
        ...
        private static $module_functions = [
            "page",
            "library",
            "tool",
            "snippet",
        ...
        public static function getAddonByID($id)
        {
            ...
            $addon = $self
                ->db()
                ->query("SELECT * FROM `:prefix:addons` WHERE addon_id=:id", [
                    "id" => $id,
                ]);
            if ($addon->rowCount() > 0) {
        ...
        public static function getAddonDetails($directory)
        {
            ...
            $addon = $self
                ->db()
                ->query("SELECT * FROM `:prefix:addons` WHERE directory=:dir", [
                    "dir" => $directory,
                ]);
            if ($addon->rowCount() > 0) {
         ...
         * @return
         **/
        public static function getAddonVariant(
            $field_to_select,
            $field_to_check,
            $check_for,
            $tablename,
            ...
            $q = $self
                ->db()
                ->query(
                    "SELECT :field_to_select: FROM `:prefix:" .
                        $tablename .
                        "` " .
        ...
        public static function get_addons(
            ...
            if (is_array($type)) {
                ...
                foreach ($type as $item) {
                    $get_type .=
                        'type = \'' . htmlspecialchars($item) . '\'' . $and;
                }
            ...
            if (is_array($function)) {
                ...
                foreach ($function as $item) {
                    $get_function .=
                        'function = \'' . htmlspecialchars($item) . '\' AND ';
                }
                $get_function = substr($get_function, 0, -5) . " )";
            } elseif ($function != "") {
                $get_function =
                    $and . 'function = \'' . htmlspecialchars($function) . '\'';
            }
            ...
            if ($addons->rowCount() > 0) {
                ...
                while ($addon = $addons->fetchRow()) {
                    if (
                        !$check_permission ||
                        ($addon["type"] != "language" &&
                            CAT_Users::get_permission(
                                $addon["directory"],
                                $addon["type"]
                            )) ||
                        $addon["type"] == "language"
                    ) {
                        $addons_array[$counter] = array_merge($addon, [
                            "VALUE" => $addon["directory"],
                            "NAME" => $addon["name"],
                            "SELECTED" =>
                                $selected == $counter ||
                                $selected == $addon["name"] ||
                                $selected == $addon["directory"]
                                    ? true
                                    : false,
                        ]);
                        $counter++;
            ...
            // reorder array
            $addons_array = CAT_Helper_Array::ArraySort(
                $addons_array,
                $order,
                "asc",
                true
         ...
         * @return boolean true on success
         */
        public static function unregister_page_title(
            $page_id,
            $module_directory
        ) {
            return unregister_addon_header(
                $page_id,
                $module_directory,
                "title"
            );
         ...
         */
        public static function is_registered_page_title(
            $page_id,
            $module_directory
        ) {
         ...
         * @return boolean true on success
         */
        public static function register_page_description(
            $page_id,
            $module_name,
            $module_directory
        ) {
         ...
         * @return boolean true on success
         */
        public static function unregister_page_description(
            $page_id,
            $module_directory
        ) {
            return unregister_addon_header(
         ...
         * @return boolen on success
         */
        public static function is_registered_page_description(
            $page_id,
            $module_directory
        ) {
            return is_registered_addon_header(
         ...
         * @return boolean true on success
         */
        public static function register_page_keywords(
            $page_id,
            $module_name,
            $module_directory
        ) {
         ...
         * @return boolean true on success
         */
        public static function unregister_page_keywords(
            $page_id,
            $module_directory
        ) {
            return unregister_addon_header(
         ...
         * @return boolen on success
         */
        public static function is_registered_page_keywords(
            $page_id,
            $module_directory
        ) {
            return is_registered_addon_header(
         ...
         */
        public static function preCheckAddon(
            $temp_addon_file,
            $temp_path = null,
            $delete_on_fail = true,
            $always_return_result = false
        ) {
            global $parser;
            ...
            foreach ($PRECHECK as $key => $value) {
                switch ($key) {
                    ...
                    case "LEPTON_VERSION":
                    case "VERSION":
                        list($status, $msg[]) = self::checkCMSVersion(
                            $key,
                            $value
                        );
                    ...
                    case "CAT_ADDONS":
                    case "WB_ADDONS":
                        list($status, $add_to_msg) = self::checkAddons(
                            $PRECHECK[$key]
                        );
                        if (count($add_to_msg)) {
                    ...
                    case "PHP_SETTINGS":
                        if (is_array($PRECHECK["PHP_SETTINGS"])) {
                            foreach (
                                $PRECHECK["PHP_SETTINGS"]
                                as $setting => $values
                            ) {
                                $actual_setting = ($temp = ini_get($setting))
                                    ? $temp
                                    : 0;
                    ...
                    case "CUSTOM_CHECKS":
                        if (is_array($PRECHECK["CUSTOM_CHECKS"])) {
                            foreach (
                                ...
                                as $custom_key => $values
                            ) {
                                $status =
                                    true === array_key_exists("STATUS", $values)
                                        ? $values["STATUS"]
                                        : false;
                                $msg[] = [
                                    "check" => $custom_key,
            ...
            $output = $parser->get("summary", [
                "addon" => isset($info["module_name"])
                    ? $info["module_name"]
                    : "",
                "heading" => $failed_checks
                    ? $self->lang()->translate("Pre installation check failed")
                    : $self
                        ->lang()
                        ->translate("Pre installation check successful"),
        ...
        public static function getVersion($version, $strip_suffix = true)
        {
            ...
            $suffix = strtoupper(
                trim(substr($revision, strlen(intval($revision))))
            );
        ...
        public static function versionCompare(
            ...
            return version_compare(
                self::getVersion2($version1),
                self::getVersion2($version2),
                $operator
            );
        } // end versionCompare()
        ...
        public static function installUploaded($tmpfile, $name)
        {
            ...
            $temp_unzip =
                $temp_dir .
                "/unzip_" .
                pathinfo($tmpfile, PATHINFO_FILENAME) .
                "/";
            ...
            if (!move_uploaded_file($tmpfile, $temp_file)) {
                ...
                self::printError(
                    "Unable to install. Cannot move uploaded file"
                );
                return false;
            }
            ...
            $self
                ...
                ->LogDebug(
                    sprintf(
                        "uploaded file was moved to [%s], call installModule()",
                        $temp_file
                    )
                );
        ...
        public static function installModule(
            ...
            $temp_unzip =
                $temp_dir .
                "/unzip_" .
                pathinfo($zipfile, PATHINFO_FILENAME) .
                "/";
            ...
            $self
                ...
                ->LogDebug(
                    sprintf(
                        "file extension [%s], source dir [%s], remove zip [%s]",
                        $extension,
                        $sourcedir,
                        $remove_zip_on_error
                    )
            ...
            if ($extension == "php") {
                ...
                $self
                    ->log()
                    ->LogDebug(
                        sprintf("creating temp. unzip dir [%s]", $temp_unzip)
                    );
                ...
                $list = CAT_Helper_Zip::getInstance($zipfile)
                    ->config(
                        "Path",
                        CAT_Helper_Directory::sanitizePath($temp_unzip)
                    )
                    ->extract();
                ...
                if (!$list) {
                    $self
                        ...
                        ->LogDebug(
                            sprintf(
                                'No $list from ZIP-Helper, removing [%s]',
                                $temp_unzip
                            )
                        );
                    CAT_Helper_Directory::removeDirectory($temp_unzip);
            ...
            // unknown extension
                ...
                $self
                    ...
                    ->LogDebug(
                        sprintf(
                            'Unknown extension [%s], "php" or "zip" expected, removing [%s]',
                            $extension,
                            $temp_unzip
                        )
                    );
            ...
            if ($addon_info = self::checkInfo($temp_infofile)) {
                $precheck_errors = self::preCheckAddon(
                    $zipfile,
                    $temp_infofile,
                    false
                );
                ...
                $self
                    ...
                    ->LogDebug(
                        sprintf(
                            "Unable to load info file [%s], removing [%s]",
                            $temp_infofile,
                            $temp_unzip
                        )
                    );
                        ...
                        $self
                            ...
                            ->translate(
                                ...
                                [
                                    "error" => $self
                                        ->lang()
                                        ->translate("Unable to find info.php"),
                                ]
                            )
                    );
                }
            ...
            if ($precheck_errors != "" && !is_bool($precheck_errors)) {
                $self
                    ...
                    ->LogDebug(
                        sprintf(
                            "Pre-installation check(s) failed, removing [%s]",
                            $temp_unzip
                        )
                    );
                CAT_Helper_Directory::removeDirectory($temp_unzip);
            ...
            if (
                ...
                if ($previous_info) {
                    ...
                    if (
                        self::versionCompare(
                            $previous_info["module_version"],
                            $addon_info["module_version"],
                            ">="
                        )
                    ) {
                        $self
                            ...
                            ->LogDebug(
                                sprintf(
                                    "Version check found no difference between installed and uploaded version, removing [%s]",
                                    $temp_unzip
                                )
                            );
                        CAT_Helper_Directory::removeDirectory($temp_unzip);
                        ...
                        if (!$silent) {
                            ...
                            self::$error =
                                "Same or newer version already installed (installed: " .
                                $previous_info["module_version"] .
                                "; to install: " .
                                $addon_info["module_version"] .
                                ")";
            ...
            if ($addon_info["addon_function"] != "language") {
                $self
                    ->log()
                    ->LogDebug(
                        sprintf("Creating addon directory [%s]", $addon_dir)
                    );
                ...
                if (
                    CAT_Helper_Directory::copyRecursive(
                        $temp_infofile,
                        $addon_dir
                    ) !== true
                ) {
                    $self
                        ->log()
                        ->LogDebug(
                            sprintf("Copy failed, removing [%s]", $temp_unzip)
                        );
                    CAT_Helper_Directory::removeDirectory($temp_unzip);
                    if ($remove_zip_on_error) {
                        CAT_Helper_Directory::removeDirectory($zipfile);
            ...
            // Switched so the precheck in upgrade.php is working correct
            ...
            if (
                !self::loadModuleIntoDB(
                    $addon_dir,
                    $action,
                    self::checkInfo($addon_dir)
                )
            ) {
                $self
                    ->log()
                    ->LogDebug(
                        sprintf(
                            "Loading module into DB failed, removing [%s]",
                            $temp_unzip
                        )
                    );
                CAT_Helper_Directory::removeDirectory($temp_unzip);
            ...
            if (
                ($addon_info["addon_function"] == "module" &&
                    ($addon_info["module_function"] == "page" ||
                        $addon_info["module_function"] == "tool")) ||
        ...
        public static function uninstallModule($type, $addon_name)
        {
            ...
            switch ($type) {
                ...
                case "modules":
                    ...
                    if (count($usage)) {
                        $temp = explode(
                            ...
                            self::getInstance()
                                ->lang()
                                ->translate("this page;these pages")
                        );
                    ...
                    if (
                        defined("WYSIWYG_EDITOR") &&
                        $addon_name == WYSIWYG_EDITOR
                    ) {
                        return self::getInstance()
                            ->lang()
                            ->translate(
                ...
                case "templates":
                    if (
                        $addon_name == DEFAULT_THEME ||
                        $addon_name == DEFAULT_TEMPLATE
                    ) {
                        $temp = [
                            "name" => $addon_name,
                            "type" =>
                                $addon_name == DEFAULT_TEMPLATE
                                    ...
                                    : self::getInstance()
                                        ->lang()
                                        ->translate("default backend theme"),
                        ];
                        return self::getInstance()
                            ->lang()
                            ->translate(
                    ...
                    if ($info->rowCount() > 0) {
                        ...
                        $page_template_str =
                            "<li><a href='../pages/settings.php?page_id={{id}}'>{{title}}</a></li>";

                        $values = [
                            "pages" => $add,
                            "name" => $addon_name,
                        ];
                        $msg = self::getInstance()
                            ->lang()
                            ->translate($msg_template_str, $values);
                        ...
                        while ($data = $info->fetchRow()) {
                            ...
                            $page_names .= self::getInstance()
                                ->lang()
                                ->translate($page_template_str, $page_info);
                        }
                        $page_names .= "</ul>";
            ...
            if ($type != "languages") {
                self::getInstance()
                    ...
                    ->query(
                        ...
                        ["dir" => $addon_name, "type" => substr($type, 0, -1)]
                    );
                if (
                    self::getInstance()
                        ->db()
                ...
                if ($stmt->rowCount() > 0) {
                    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                        ...
                        $permissions = explode(
                            ",",
                            $row[substr($type, 0, -1) . "_permissions"]
                        );
                        ...
                        if (in_array($file, $permissions)) {
                            ...
                            self::getInstance()
                                ...
                                ->query(
                                    ...
                                    ["perm" => $addon_permissions, "id" => $gid]
                                );
                        }
                    }
                }
                // Try to delete the module dir
                $mod_dir = CAT_Helper_Directory::sanitizePath(
                    CAT_PATH . "/" . $type . "/" . $addon_name
                );
                if (
         ...
         * @return
         **/
        public static function loadModuleIntoDB(
            $addon_dir,
            $action,
            $addon_info = []
        ) {
            ...
            $self
                ->log()
                ->logDebug(
                    sprintf("addon dir [%s], action [%s]", $addon_dir, $action)
                );
            ...
            if ($action == "install") {
                if (isset($addon_info["module_name"])) {
                    ...
                    $options = [
                        ...
                        "plat" => isset($addon_info["module_platform"])
                            ? $addon_info["module_platform"]
                            : "",
                        "auth" => isset($addon_info["module_author"])
                            ? $addon_info["module_author"]
                            : "",
                        "lic" => isset($addon_info["module_license"])
                            ? $addon_info["module_license"]
                            : "",
                        "guid" => isset($addon_info["module_guid"])
                            ? $addon_info["module_guid"]
                            : "",
                    ];

                    $module_function = strtolower(
                        $addon_info["module_function"]
                    );
                    $do = "insert";
                    ...
                    if ($cnt > 0) {
                        $sql =
                            "UPDATE `:prefix:addons` SET `upgraded`=:time, " .
                        ...
                        $sql =
                            "INSERT INTO `:prefix:addons` VALUES " .
        ...
        public static function setModulePermissions($addon_info)
        {
            ...
            $stmt = $self
                ->db()
                ->query("SELECT * FROM `:prefix:groups` WHERE group_id <> 1");
            ...
            if ($info->rowCount() > 0) {
                ...
                for ($i = count($usage) - 1; $i >= 0; $i--) {
                    ...
                    if (substr($usage[$i]["page_id"], 0, 1) == "-") {
                        ...
                        $usage[$i]["page_link"] = CAT_Helper_Page::getLink(
                            $usage[$i]["page_id"]
                        );
                        $usage[$i]["menu_title"] = CAT_Helper_Page::properties(
                            $usage[$i]["page_id"],
                            "menu_title"
                        );
                    }
        ...
        public static function getModuleVersion($modulename, $source = true)
        {
            ...
            if ($source != true) {
                $sql =
                    "SELECT `version` FROM `:prefix:addons` WHERE `directory`=:dir";
                $version = $self
                    ->db()
                    ->query($sql, ["dir" => $modulename])
                    ->fetchColumn();
            } else {
        ...
        public static function upgradeModule($directory, $upgrade = false)
        {
            ...
            if ($self->db()->isError()) {
                ...
                self::getInstance()
                    ->log()
                    ->logDebug("database error: " . $self->db()->getError());
            }
        ...
        public static function isModuleInstalled(
            ...
            $q = $self
                ...
                ->query(
                    ...
                    ["type" => $type, "dir" => $module, "name" => $module]
                );
            ...
            while ($addon = $q->fetchRow()) {
                if (
                    $version &&
                    self::versionCompare($addon["version"], $version)
                ) {
                    return true;
                }
        ...
        public static function sec_register_file($module, $filepath)
        {
            ...
            if (
                ...
                self::getInstance()
                    ->log()
                    ->logCrit(
                        "sec_register_file() called outside admin context!"
                    );
                self::$error =
                    "sec_register_file() called outside admin context!";
                return false;
            ...
            // check permissions
            if (
                !CAT_Users::checkPermission("Addons", "modules_install") &&
                !defined("CAT_INSTALL")
            ) {
                self::getInstance()
                    ->log()
                    ->logCrit(
                        "sec_register_file() called without modules_install perms!"
                    );
                self::$error =
                    "sec_register_file() called without modules_install perms!";
                return false;
            ...
            if (!is_dir(CAT_PATH . "/modules/" . $module)) {
                self::getInstance()
                    ->log()
                    ->logCrit(
                        "sec_register_file() called for non existing module [$module] (path: [$filepath])"
                    );
            ...
            if (
                !file_exists(
                    CAT_Helper_Directory::sanitizePath(
                        CAT_PATH . "/modules/" . $module . "/" . $filepath
                    )
                )
            ) {
                self::getInstance()
                    ->log()
                    ->logCrit(
                        "sec_register_file() called for non existing file [$filepath] (module: [$module])"
                    );
            ...
            if (!$q->rowCount()) {
                self::getInstance()
                    ->log()
                    ->logCrit(
                        "sec_register_file() called for non existing module [$module] (path: [$filepath]) - not found in addons table!"
                    );
        ...
        public static function checkInfo($directory)
        {
            ...
            $self
                ->log()
                ->LogDebug(
                    sprintf('checking info.php for $directory [%s]', $directory)
                );
            if (is_dir($directory) && file_exists($directory . "/info.php")) {
                $self
                    ->log()
                    ->LogDebug('$directory is a directory and info.php found');
                // get header info
                $link = null;
                ...
                foreach (
                    self::$info_vars_mandatory[$return_values["addon_function"]]
                    as $varname
                ) {
                    if (!isset(${$varname})) {
                        self::$error =
                            "Invalid info.php - mandatory var " .
                            $varname .
                            " not set";
                        $self->log()->logDebug(self::$error);
                        return false;
                ...
                // add empty keys
                foreach (
                    self::$info_vars_full[$return_values["addon_function"]]
                    as $varname
                ) {
                    $key = str_ireplace(["template_"], ["module_"], $varname);
                ...
                pathinfo($directory, PATHINFO_EXTENSION) == "php"
            ) {
                $self
                    ->log()
                    ->LogDebug('$directory is a file and has "php" suffix');
                // Check if the file is valid
        ...
        public static function getEditModuleCSSForm($mod_dir, $print = false)
        {
            ...
            if (
                !file_exists(CAT_PATH . "/backend/addons/edit_module_files.php")
            ) {
                return;
            }
            if (!file_exists(CAT_PATH . "/modules/" . $mod_dir . "/info.php")) {
                return;
            }
            ...
            $buttons = [];
            $content = "";
            foreach (["frontend.css", "backend.css"] as $file) {
                if (
                    file_exists(CAT_PATH . "/modules/" . $mod_dir . "/" . $file)
                ) {
                    $buttons[] = $file;
                }
            }
            if (count($buttons)) {
                foreach ($buttons as $button) {
                    $content .=
                        '
        			<form class="edit_module_file" name="edit_module_file" action="' .
                        CAT_URL .
                        '/modules/edit_module_files.php" method="post">
                        ...
                        $section_id .
                        '" />
        				<input type="hidden" name="mod_dir" value="' .
                        $mod_dir .
                        '" />
        				...
        				<button type="submit" class="edit_module_file mod_' .
                        $mod_dir .
                        '_edit_css">' .
                        self::getInstance()
                            ->lang()
         ...
         * @return
         **/
        public static function getEditModuleFilesButton(
            $mod_dir,
            $print = false
        ) {
            global $page_id, $section_id;
            ...
            if (count($js) || count($css)) {
                $content =
                    ...
                    CAT_URL .
                    '/backend/addons/edit_module_files.php" name="edit_module_file">
                    ...
                    $self->lang()->translate("Edit module file(s)") .
                    '">
                	</form>';
            }
            if ($print) {
            ...
            if (is_array($addons)) {
                ...
                foreach ($addons as $addon => $values) {
                    if (is_array($values)) {
                        ...
                        $version =
                            isset($values["VERSION"]) &&
                            trim($values["VERSION"]) != ""
                                ? $values["VERSION"]
                                : "";
                        $operator =
                            isset($values["OPERATOR"]) &&
                            trim($values["OPERATOR"]) != ""
                                ? $values["OPERATOR"]
                                : ">=";
                    } else {
        ...
        private static function checkCMSVersion($key, $value)
        {
            ...
            // obtain operator for string comparison if exist
            $operator =
                isset($value["OPERATOR"]) && trim($value["OPERATOR"]) != ""
                    ? $value["OPERATOR"]
                    : ">=";
            // compare versions and extract actual status
            $status = self::versionCompare(
