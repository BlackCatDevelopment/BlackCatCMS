...
if (!class_exists("CAT_Helper_Page")) {
    ...
    class CAT_Helper_Page extends CAT_Object
    {
        protected $_config = [
            "loglevel" => 8,
            "forbidden_l0" => [
                // configurables will be added later
                "account",
                "framework",
                "include",
                "install",
        ...
        private static $pages_by_visibility = [];
        ...
        private static $js_search_path = [];

        // footer components
        private static $script = [];
        private static $f_jquery = [];
        private static $f_js = [];
        ...
        private static function init($force=false)
        {
            ...
            if (
                CAT_Registry::exists("CAT_HELPER_PAGE_INITIALIZED") &&
                !$force
            ) {
                return;
            }
            ...
            foreach (
                [PAGES_DIRECTORY, MEDIA_DIRECTORY, CAT_BACKEND_FOLDER]
                as $dir
            ) {
            ...
            if (count(self::$pages) == 0 || $force) {
                ...
                if ($result && $result->rowCount() > 0) {
                    ...
                    while (false !== ($row = $result->fetch())) {
                        ...
                        $row["is_open"] = false;
                        $row["be_tree_is_open"] = isset(
                            $_COOKIE[
                                session_name() . "pageid_" . $row["page_id"]
                            ]
                        )
                            ? true
                            : false; // for page tree
                    ...
                    foreach (self::$pages as $i => $page) {
                        if (isset($children_count[$page["page_id"]])) {
                            self::$pages[$i]["children"] =
                                $children_count[$page["page_id"]];
                            self::$pages[$i]["is_parent"] = true;
                            self::$pages[$i]["has_children"] = true;
                        }
                        if (
                            $direct_parent &&
                            $page["page_id"] == $direct_parent
                        ) {
...
// --------------------- NOT READY YET! ----------------------------------------
                    ...
                    if (
                        ...
                        foreach ($trail as $id) {
                            if (
                                isset(self::$pages_by_id[$id]) &&
                                isset(self::$pages[self::$pages_by_id[$id]])
                            ) {
        ...
        public static function addPage($options)
        {
            ...
            $res = $self->db()->query("DESCRIBE `:prefix:pages`");
            $mandatory = [];
            ...
            $auto_add = CAT_Helper_Validate::sanitizePost(
                "auto_add_modules",
                null,
                true
            )
            ...
            // reload pages list
                ...
                if (
                    isset($auto_add_modules) &&
                    is_array($auto_add_modules) &&
        ...
        public static function createAccessFile($filename, $page_id)
        {
                ...
                !preg_match(
                    "~^" .
                        CAT_Helper_Directory::sanitizePath(
                            CAT_PATH . PAGES_DIRECTORY
                        ) .
                        "~i",
                    $filename
                )
            ) {
                $filename = CAT_Helper_Directory::sanitizePath(
                    CAT_PATH .
                        PAGES_DIRECTORY .
                        "/" .
                        dirname($filename) .
                        "/" .
                        self::getFilename(basename($filename)) .
                        PAGE_EXTENSION
                );
            }

            $pages_path = CAT_Helper_Directory::sanitizePath(
                CAT_PATH . PAGES_DIRECTORY
            );
            $rel_pages_dir = str_replace(
                $pages_path,
                "",
                CAT_Helper_Directory::sanitizePath(dirname($filename))
            );
            $rel_filename = str_replace(
                $pages_path,
                "",
            ...
            if (true === is_writable($pages_path) && false == $denied) {
                ...
                $content = "<?php" . "\n";
                $content .=
                    "/**\n *\tThis file is autogenerated by BlackCat CMS Version " .
                    CAT_VERSION .
                    "\n";
                ...
                if ($fp) {
                    ...
                    CAT_Backend::getInstance()->print_error(
         ...
         * @return
         **/
        public static function getExtraHeaderFiles(
            $page_id = null,
            $db_only = false
        ) {
            $data = []; //'js'=>array(),'css'=>array(),'code'=>''
            $q =
                "SELECT * FROM `:prefix:pages_headers` WHERE `page_id`=:page_id";
            $r = self::getInstance(1)
                ->db()
                ->query($q, ["page_id" => $page_id]);
            if ($r->rowCount()) {
                $row = $r->fetch();
                if (
                    isset($row["page_js_files"]) &&
                    $row["page_js_files"] != ""
                ) {
            ...
            if (defined("WYSIWYG_EDITOR") && WYSIWYG_EDITOR != "") {
                $cfg_file = CAT_Helper_Directory::sanitizePath(
                    CAT_PATH . "/modules/" . WYSIWYG_EDITOR . "/c_editor.php"
                );
         ...
         * @return array
         **/
        public static function adminAddHeaderComponent(
            $type,
            $file,
            $page_id = null
        ) {
            ...
            if (
                ...
                is_array($data[$type]) &&
                count($data[$type]) &&
                in_array($file, $data[$type])
            ) {
                return [
                    "success" => false,
                    "message" => $self
                        ->lang()
                        ->translate("The file is already listed"),
                ];
            } else {
                $paths = [
                    CAT_Helper_Directory::sanitizePath(
                ...
                foreach ($paths as $path) {
                    ...
                    $new = array_unique($new);
                        ...
                        if (count($data)) {
                            $q =
                                "UPDATE `:prefix:pages_headers` SET :field:=:value WHERE `page_id`=:page_id";
                        } else {
                            $q =
                        ...
                        return [
                            "success" => self::getInstance(1)->isError()
                                ? false
                                : true,
                            "message" => self::getInstance(1)->isError()
            ...
            return [
                "success" => false,
                "message" =>
                    $self->lang()->translate("Unable to add the file") .
                    " " .
                    $file,
            ];
         ...
         **/
        public static function adminDelHeaderComponent(
            $type,
            $file,
            $page_id = null
        ) {
        ...
        public static function getBackendFooters()
        {
            ...
            $file = CAT_Helper_Directory::sanitizePath(
                CAT_PATH . "/templates/" . $tpl . "/footers.inc.php"
            );
            if (file_exists($file)) {
                self::getInstance()
                    ->log()
                    ->logDebug(
                        sprintf(
                            "adding footer items for backend theme [%s]",
                            $tpl
                        )
                    );
            ...
            if ($tool) {
                $path = CAT_Helper_Directory::sanitizePath(
                    CAT_PATH . "/modules/" . $tool . "/tool.php"
                );
                self::getInstance()
                    ->log()
                    ->logDebug(
                        sprintf(
                            "handle admin tool [%s] - path [%s]",
                            $tool,
                            $path
                        )
                    );
                if (file_exists($path)) {
                    $file = CAT_Helper_Directory::sanitizePath(
                        CAT_PATH . "/modules/" . $tool . "/footers.inc.php"
                    );
                    if (file_exists($file)) {
                        self::getInstance()
                            ->log()
                            ->logDebug(
                                sprintf(
                                    "adding footer items for admin tool [%s]",
                                    $_REQUEST["tool"]
                                )
                            );
                        self::_load_footers_inc(
                            $file,
                            "backend",
                            "modules/" . $tool
                        );
        ...
        public static function getBackendHeaders($section)
        {
            ...
            $self
                ->log()
                ->logDebug("get backend headers for section:", $section);

            // -----------------------------------------------------------------
            // -----                    backend theme                      -----
            // -----------------------------------------------------------------
            $file = CAT_Helper_Directory::sanitizePath(
                CAT_PATH . "/templates/" . DEFAULT_THEME . "/headers.inc.php"
            );
            if (file_exists($file)) {
                self::$instance
                    ->log()
                    ->logDebug(
                        sprintf(
                            "adding items for backend theme [%s]",
                            DEFAULT_THEME
                        )
                    );
            ...
            if (isset($_REQUEST["tool"])) {
                $path = CAT_Helper_Directory::sanitizePath(
                    CAT_PATH . "/modules/" . $_REQUEST["tool"] . "/tool.php"
                );
                self::$instance
                    ->log()
                    ->logDebug(
        ...
        public static function getFrontendFooters()
        {
            ...
            if (file_exists($file)) {
                self::$instance
                    ...
                    ->logDebug(
                        sprintf(
                            "adding footer items for frontend template [%s]",
                            $tpl
                        )
                    );
            ...
            if (count(CAT_Helper_Page::$js_search_path)) {
                ...
                foreach (CAT_Helper_Page::$js_search_path as $directory) {
                    $file = CAT_Helper_Directory::sanitizePath(
                        $directory . "/frontend_body.js"
                    );
                    if (file_exists(CAT_PATH . "/" . $file)) {
                        CAT_Helper_Page::$f_js[] =
        ...
        public static function getFrontendHeaders()
        {
            ...
            $tpl = CAT_Registry::get("TEMPLATE");
            $file = CAT_Helper_Directory::sanitizePath(
                CAT_PATH . "/templates/" . $tpl . "/headers.inc.php"
            );
            self::$instance
                ->log()
                ->logDebug(sprintf("searching for file [%s]", $file));
            if (file_exists($file)) {
                self::$instance
                    ->log()
                    ->logDebug(
                        sprintf("adding items for frontend template [%s]", $tpl)
                    );
            ...
            // called from backend?
            if (
                CAT_Helper_Validate::get("_REQUEST", "preview") &&
                CAT_Users::is_authenticated()
            ) {
                $file =
                    CAT_PATH .
                    "/templates/" .
                ...
                if (file_exists($file)) {
                    ...
                    CAT_Helper_Page::$js[] =
                        ...
                        self::getInstance(1)
                            ->lang()
                            ->translate(
                                self::properties($page_id, "visibility")
                            ) .
                        '\';' .
                        "\n" .
                        '    var visibility_text = \'' .
                        self::getInstance(1)
                            ->lang()
                            ->translate("Visibility of this page") .
                        '\';' .
                        "\n" .
                        '    var visibility_title = \'' .
                        self::getInstance(1)
                            ->lang()
                            ->translate("Black Cat CMS Page Preview") .
                        '\';' .
                        "\n" .
                        "</script>" .
                        "\n" .
            ...
            $droplets_config = CAT_Helper_Droplet::getDropletsForHeader(
                $page_id
            );
        ...
        public static function getHeaders($for = null, $section = false)
        {
            ...
            if (defined("CAT_HEADERS_SENT")) {
                self::$instance
                    ->log()
                    ->logDebug("headers already sent, returning");
                return;
            }
        ...
        var THEME_URL                         = '" .
            ...
            CAT_Helper_Page::$jquery[] = "<script>" . $default . "</script>";
        ...
        {
            $sql =
                "SELECT * FROM `:prefix:page_langs` AS t1" .
         ...
         * @return HTML
         **/
        public static function getDefaultMeta($droplets_config = [])
        {
            global $page_id;
            ...
            $output[] =
                ...
                (isset($properties["default_charset"]) &&
            ...
            if (
                isset($properties["settings"]) &&
                isset($properties["settings"]["seo"])
            ) {
                ...
                if (isset($seo["robots"]) && isset($seo["robots"][0])) {
                    ...
                    if (!array_key_exists("noindex", $check)) {
                        $robots[] = "index";
                    }
         ...
         * @return
         **/
        public static function getMeta($droplets_config = [])
        {
            global $page_id;
            $meta = self::getDefaultMeta($droplets_config);
            self::$meta = array_merge($meta, self::$meta);
            ...
            if (is_array($linked) && count($linked)) {
                ...
                foreach ($linked as $page) {
                    $alt[] =
                        ...
                        strtolower($page["lang"]) .
                        '" href="' .
                        self::getLink($page["page_id"]) .
                        '">';
                }
        ...
        public static function getPagePermission($page_id, $action = "admin")
        {
            ...
            if (
                ...
                is_numeric(
                    array_search(
                        CAT_Users::getInstance()->get_user_id(),
                        $users
                    )
                )
            ) {
        ...
        public static function getPageSelect($as_array=false)
        {
            ...
            if ($as_array) {
                ...
                return CAT_Helper_ListBuilder::getInstance()
                    ->config([
                        "__li_level_css" => true,
            ...
            if ($type) {
                if ($key) {
                    if (isset($set[$type][$key])) {
                        if (
                            is_array($set[$type][$key]) &&
                            count($set[$type][$key]) == 1
                        ) {
        ...
        public static function getPageByPath($path)
        {
            ...
            $path_without_suffix = str_replace(
         ...
         * @return array
         **/
        public static function getPagesByParent(
            $parent = 0,
            $add_sections = false
        ) {
         ...
         * @return
         **/
        public static function getPageTrail(
            $page_id,
            $skip_zero = false,
            $as_array = false
        ) {
        ...
        public static function getSections($page_id = null)
        {
            ...
            if ($page_id) {
                return isset(self::$pages_sections[$page_id])
         ...
         * @return
         **/
        public static function getSubPages($page_id, $result = [])
        {
        ...
        public static function getVirtualPage($title)
        {
            ...
            define(
                "PAGE_TITLE",
                CAT_Helper_I18n::getInstance()->translate($title)
            );
            define(
                "MENU_TITLE",
                CAT_Helper_I18n::getInstance()->translate($title)
        ...
        public static function sanitizeLink(&$page)
        {
            ...
            // sub level
                ...
                $parent_section = "";
                $parent_titles = array_reverse(
                    self::getParentTitles($page["parent"])
                );
        ...
        public static function selectPage( $no_intro = false )
        {
            ...
            if (
                INTRO_PAGE &&
                !$no_intro &&
                (!isset($page_id) || !is_numeric($page_id)) &&
                !(
                    CAT_Registry::get("USE_SHORT_URLS") &&
            ...
            if (!$page_id) {
                if (
                    ...
                    $page_id = CAT_Helper_Page::getPageByPath(
                        "/" . strtok($_SERVER["REDIRECT_QUERY_STRING"], "&")
                    );
                } else {
                    $page_id = self::getDefaultPage();
            }
        ...
        public static function printUnderConstruction()
        {
            ...
            $file = CAT_Helper_Directory::getInstance()->findFile(
                "under_construction",
            ...
            if (!$file) {
                ...
                $image_url =
                    CAT_URL . "/templates/" . DEFAULT_THEME . "/images";
            }
            if (!$file) {
...
#echo "page_id $page_id\n";
        ...
        public static function isActive($page_id)
        {
            ...
            if (
                ...
                count(self::$pages_sections[$page_id])
            ) {
                return true;
            }
            return false;
            ...
            switch ($page["visibility"]) {
                ...
                case "registered":
                    if (CAT_Users::is_authenticated() == true) {
                        // check language
                        if (
                            CAT_Registry::get("PAGE_LANGUAGES") == "false" ||
                            (self::properties($page_id, "language") == "" ||
                                self::properties($page_id, "language") ==
                                    LANGUAGE)
                        ) {
                            $show_it =
                                CAT_Users::is_group_match(
                                    CAT_Users::get_groups_id(),
                                    $page["viewing_groups"]
                                ) ||
                                CAT_Users::is_group_match(
                                    CAT_Users::get_user_id(),
                                    $page["viewing_users"]
                                ) ||
                                CAT_Users::is_root();
                    }
...
// *****************************************************************************
        ...
        private static function _analyze_css(&$arr, $path_prefix = null)
        {
            if (is_array($arr)) {
                ...
                foreach ($arr as $css) {
                    ...
                    // relative path?
                    if (
                        !preg_match("#/modules/#i", $css["file"]) &&
                        !!preg_match("#/templates/#i", $css["file"])
                    ) {
                        foreach ($check_paths as $subdir) {
                            if (
                                !preg_match("#" . $subdir . "/#", $css["file"])
                            ) {
                                $css[
                                    "file"
                                ] = CAT_Helper_Directory::sanitizePath(
         ...
         * @return void
         **/
        private static function _analyze_javascripts(
            &$arr,
            $for = "header",
            $path_prefix = null,
            $section = false
         ...
         * @return void
         **/
        private static function _analyze_jquery_components(
            &$arr,
            $for = "frontend",
            $section = null
        ) {
            ...
            if (
                ...
                file_exists(
                    CAT_PATH .
                        "/modules/lib_jquery/jquery-ui/themes/" .
                        $arr["ui-theme"]
                )
            ) {
                $static[] =
            ...
            if (
                !CAT_Helper_Page::$jquery_core &&
                isset($arr["core"]) &&
                $arr["core"] === true
            ) {
                CAT_Helper_Page::$jquery[] =
                    CAT_Helper_Page::$space .
                    '<script src="' .
                    $val->sanitize_url(
                        CAT_URL .
            ...
            if (
                !CAT_Helper_Page::$jquery_ui_core &&
                isset($arr["ui"]) &&
                $arr["ui"] === true
            ) {
                CAT_Helper_Page::$jquery[] =
                    ...
                    $val->sanitize_url(
                        CAT_URL .
                            "/modules/lib_jquery/jquery-ui/ui/jquery-ui.min.js"
                    ) .
                    '"></script>' .
                    "\n";
                CAT_Helper_Page::$jquery[] =
                    ...
                    $val->sanitize_url(
                        CAT_URL .
                            "/modules/lib_jquery/jquery-ui/ui/i18n/jquery-ui-i18n.min.js"
                    ) .
                    '"></script>' .
                    "\n";
                ...
                foreach ($arr["individual"] as $section_name => $item) {
                    if ($section_name == strtolower($section)) {
                        ...
                        $static[] =
                            ...
                            '<script src="' .
                            $val->sanitize_url(
                                CAT_URL . "/modules/lib_jquery/plugins/" . $item
                            ) .
                            '"></script>' .
                            "\n";
                    }
        ...
        private static function _deletePage($page_id)
        {
            ...
            $self
                ->db()
                ->query("DELETE FROM `:prefix:pages` WHERE `page_id` = :id", [
                    "id" => $page_id,
                ]);
            if ($self->db()->isError()) {
        ...
        private static function _find_item($item)
        {
            ...
            if (
                !file_exists(
                    CAT_Helper_Directory::sanitizePath(
                        CAT_PATH . "/modules/lib_jquery/plugins/" . $item
                    )
                )
            ) {
         ...
         * @return void
         **/
        private static function _load_footers_inc(
            $file,
            $for,
            $path_prefix,
            $section = null
            ...
            // analyze
            if (
                isset($mod_footers[$for]) &&
                is_array($mod_footers[$for]) &&
                count($mod_footers[$for])
            ) {
                if (
                    isset($mod_footers[$for]["jquery"]) &&
                    is_array($mod_footers[$for]["jquery"]) &&
                    count($mod_footers[$for]["jquery"])
                ) {
                    self::_analyze_jquery_components(
                        $mod_footers[$for]["jquery"][0],
                        $for,
                        $section
                    );
                }
                // ----- other JS -----
                if (
                    isset($mod_footers[$for]["js"]) &&
                    is_array($mod_footers[$for]["js"]) &&
                    count($mod_footers[$for]["js"])
                ) {
                    $temp_arr = is_array($mod_footers[$for]["js"][0])
                        ? $mod_footers[$for]["js"][0]
                        : $mod_footers[$for]["js"];
                    self::_analyze_javascripts(
                        $mod_footers[$for]["js"],
                        "footer",
                        $path_prefix . "/js",
                        $section
                ...
                self::$instance
                    ->log()
                    ->logDebug(sprintf('no $mod_footers for [%s]', $for));
            }
         ...
         * @return void
         **/
        private static function _load_headers_inc(
            $file,
            $for,
            $path_prefix,
            $section = null
            ...
            // analyze
            if (
                isset($mod_headers[$for]) &&
                is_array($mod_headers[$for]) &&
                count($mod_headers[$for])
            ) {
                // ----- META -----
                if (
                    isset($mod_headers[$for]["meta"]) &&
                    is_array($mod_headers[$for]["meta"]) &&
                    count($mod_headers[$for]["meta"])
                ) {
                    self::_analyze_meta($mod_headers[$for]["meta"]);
                }
                // ----- CSS -----
                if (
                    isset($mod_headers[$for]["css"]) &&
                    is_array($mod_headers[$for]["css"]) &&
                    count($mod_headers[$for]["css"])
                ) {
                    self::_analyze_css($mod_headers[$for]["css"], $path_prefix);
                }
                // ----- jQuery -----
                if (
                    isset($mod_headers[$for]["jquery"]) &&
                    is_array($mod_headers[$for]["jquery"]) &&
                    count($mod_headers[$for]["jquery"])
                ) {
                    self::_analyze_jquery_components(
                        $mod_headers[$for]["jquery"],
                        $for,
                        $section
                    );
                }
                // ----- other JS -----
                if (
                    isset($mod_headers[$for]["js"]) &&
                    is_array($mod_headers[$for]["js"]) &&
                    count($mod_headers[$for]["js"])
                ) {
                    $temp_arr = is_array($mod_headers[$for]["js"][0])
                        ? $mod_headers[$for]["js"][0]
                        : $mod_headers[$for]["js"];
                    self::_analyze_javascripts(
                        $temp_arr,
                        "header",
        ...
        private static function _load_js($for = "frontend")
        {
            ...
            if (count(CAT_Helper_Page::$js_search_path)) {
                ...
                if ($for == "frontend") {
                    $file = CAT_Helper_Directory::sanitizePath(
                        CAT_Registry::get("PAGES_DIRECTORY") .
                            "/js/" .
                            $page_id .
                            ".js"
                    );
        ...
        private static function _load_sections($for = "frontend")
        {
            ...
            if (!$page_id) {
                $page_id = CAT_Helper_Validate::get(
                    "_REQUEST",
                    "page_id",
                    "numeric"
                );
            ...
            if ($page_id && is_numeric($page_id)) {
                        ...
                        foreach ($item as $section) {
                            ...
                            if (file_exists($file)) {
                                self::$instance
                                    ...
                                    ->logDebug(
                                        sprintf(
                                            "loading headers.inc.php for module [%s]",
                                            $module
                                        )
                                    );
                ...
                if (!$wysiwyg_seen) {
                    if (
                        ...
                        self::$instance
                            ->log()
                            ->logDebug("adding headers.inc.php for wysiwyg");
                        self::_load_headers_inc(
                            CAT_Helper_Directory::sanitizePath(
                                CAT_PATH .
                                    "/modules/" .
                ...
                if (
                    $for == "frontend" &&
                    CAT_Registry::get("SHOW_SEARCH") === true
                ) {
                    array_push(
                        CAT_Helper_Page::$js_search_path,
                        "/modules/" .
