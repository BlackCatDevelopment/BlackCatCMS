...
require dirname(__FILE__) .
    "/../../../modules/lib_doctrine/Doctrine/Common/ClassLoader.php";
    ...
    class CAT_Helper_DB
    {
        ...
        public function __construct($opt = [])
        {
            ...
            if (!$this->classLoader) {
                $this->classLoader = new ClassLoader(
                    "Doctrine",
                    dirname(__FILE__) . "/../../../modules/lib_doctrine"
                );
        ...
        public static function check()
        {
            if (self::$conn && is_object(self::$conn)) {
                try {
                    self::$conn->executeQuery("SHOW TABLES");
                    return true;
                } catch (Exception $e) {
                    return false;
                }
        ...
        public static function connect($opt = [])
        {
            ...
            if (!self::$conn) {
                ...
                $connectionParams = [
                    "charset" => "utf8",
                    "driver" => "pdo_mysql",
                    "dbname" => isset($opt["DB_NAME"])
                        ? $opt["DB_NAME"]
                        : "blackcat",
                    "host" => isset($opt["DB_HOST"])
                        ? $opt["DB_HOST"]
                        : "localhost",
                    "password" => isset($opt["DB_PASSWORD"])
                        ? $opt["DB_PASSWORD"]
                        : "",
                    "user" => isset($opt["DB_USERNAME"])
                        ? $opt["DB_USERNAME"]
                        : "root",
                ...
                try {
                    self::$conn = \Doctrine\DBAL\DriverManager::getConnection(
                        $connectionParams,
                        $config
                    );
        ...
        public function query($sql, $bind = [])
        {
            ...
            try {
                if (is_array($bind)) {
                    ...
                    $sql = str_replace(":prefix:", self::$prefix, $sql);
                    $stmt = self::$conn->executeQuery($sql);
                }
                self::restoreExceptionHandler();
         ...
         *
         **/
        public static function sqlImport(
            $import,
            $replace_prefix = null,
            $replace_with = null
        ) {
        ...
        {
            $prevhandler = set_exception_handler([
                "CAT_PDOExceptionHandler",
                "exceptionHandler",
            ]);
            if (
                isset($prevhandler[0]) &&
                $prevhandler[0] == "CAT_PDOExceptionHandler"
            ) {
...
class CAT_PDOStatementDecorator
{
    ...
    {
        return call_user_func_array([$this->pdo_stmt, $method], $args);
    }
...
class CAT_PDOExceptionHandler
{
    ...
    public static function exceptionHandler($exception)
    {
        if (CAT_Helper_DB::$exc_trace === true) {
            $traceline = "#%s %s(%s): %s(%s)";
            $msg =
                "Uncaught exception '%s' with message '%s'<br />" .
