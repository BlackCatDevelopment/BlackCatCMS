  ...
  class CAT_Backend extends CAT_Object
  {
    ...
    public static function getInstance(
      ...
      if (!self::$instance) {
        ...
        $parser->setGlobals(
          "TEMPLATE_MENU",
          CAT_Helper_Template::get_template_menus()
        );
        // Auto header code
        if ($auto_header == true) {
    ...
    public static function print_banner()
    {
      ...
      $tpl_data["MODIFIED_WHEN"] =
        $results_array["modified_when"] != 0
          ? ($modified_ts = CAT_Helper_DateTime::getDateTime(
            $results_array["modified_when"]
          ))
          : false;
      $tpl_data["PAGE_HEADER"] = self::getInstance("")
        ->lang()
        ->translate("Modify page");
      $tpl_data["CUR_TAB"] = "modify";
      $tpl_data["PAGE_LINK"] = CAT_Helper_Page::getLink(
        $results_array["page_id"]
      );
    ...
    public function print_header()
    {
      ...
      // =================================
      $tpl_data["permission"]["pages"] = $user->checkPermission(
        "pages",
        "pages",
        false
      );
      $tpl_data["permission"]["pages_add"] = $user->checkPermission(
        "pages",
        "pages_add",
        false
      );
      $tpl_data["permission"]["pages_add_l0"] = $user->checkPermission(
        "pages",
        "pages_add_l0",
        false
      );
      $tpl_data["permission"]["pages_modify"] = $user->checkPermission(
        "pages",
        "pages_modify",
        false
      );
      $tpl_data["permission"]["pages_delete"] = $user->checkPermission(
        "pages",
        "pages_delete",
        false
      );
      $tpl_data["permission"]["pages_settings"] = $user->checkPermission(
        "pages",
        "pages_settings",
        false
      );
      $tpl_data["permission"]["pages_intro"] =
        $user->checkPermission("pages", "pages_intro", false) != true ||
        INTRO_PAGE != "enabled"
          ? false
          : true;

      if ($tpl_data["permission"]["pages"] == true) {
        $tpl_data["DISPLAY_MENU_LIST"] =
          CAT_Registry::get("MULTIPLE_MENUS") != false ? true : false;
        $tpl_data["DISPLAY_LANGUAGE_LIST"] =
          CAT_Registry::get("PAGE_LANGUAGES") != false ? true : false;
        $tpl_data["DISPLAY_SEARCHING"] =
          CAT_Registry::get("SEARCH") != false ? true : false;
        ...
        foreach ($pages as $i => $page) {
          ...
          if (
            isset($sections[$page["page_id"]]) &&
            count($sections[$page["page_id"]])
          ) {
            $page["page_title"] .=
              "\n" .
              count($sections[$page["page_id"]]) .
              " " .
              $user->lang()->translate("active sections") .
              ":";
          ...
          $text = $parser->get(
            ...
            array_merge(array_merge($page, $tpl_data), [
              "action" =>
                pathinfo($uri, PATHINFO_FILENAME) == "lang_settings"
                  ? "lang_settings"
                  : "modify",
            ])
          );
        ...
        $tpl_data["pages"] = CAT_Helper_ListBuilder::getInstance()
          ->config([
            "__li_level_css" => true,
        ...
        // ==========================================
        $tpl_data["templates"] = $addons->get_addons(
          CAT_Registry::get("DEFAULT_TEMPLATE"),
          "template",
          "template"
        );
        $tpl_data["languages"] = $addons->get_addons(
        ...
        foreach ($tpl_data["templates"] as $key => $template) {
          $tpl_data["templates"][$key]["permissions"] = $user->get_permission(
            $template["VALUE"],
            "template"
          )
            ? true
      ...
      // =========================
      $tpl_data["META"]["CHARSET"] =
        true === defined("DEFAULT_CHARSET") ? DEFAULT_CHARSET : "utf-8";
      ...
      $date_search = ["Y", "j", "n", "jS", "l", "F"];
      $date_replace = ["yy", "y", "m", "d", "DD", "MM"];
      $tpl_data["DATE_FORMAT"] = str_replace(
        $date_search,
        $date_replace,
        CAT_Registry::get("CAT_DATE_FORMAT")
      );

      $time_search = ["H", "i", "s", "g"];
      $time_replace = ["hh", "mm", "ss", "h"];
      $tpl_data["TIME_FORMAT"] = str_replace(
        $time_search,
        $time_replace,
        CAT_Registry::get("CAT_TIME_FORMAT")
      );

      $tpl_data["SESSION"] = session_name();

      $tpl_data["HEAD"]["SECTION_NAME"] = $this->lang()->translate(
        strtoupper(self::$instance->section_name)
      );
      ...
      $tpl_data["PREFERENCES"] = [
        ...
        "permission" => $this->get_link_permission("preferences")
          ...
          : false,
        "current" =>
          "preferences" == strtolower($this->section_name) ? true : false,
      ];

      $tpl_data["section_name"] = strtolower($this->section_name);
      $tpl_data["page_id"] =
        CAT_Helper_Validate::sanitizeGet("page_id", "numeric") &&
        CAT_Helper_Validate::sanitizeGet("page_id") != ""
          ? CAT_Helper_Validate::sanitizeGet("page_id")
          : (CAT_Helper_Validate::sanitizePost("page_id", "numeric") &&
    ...
    public static function print_footer()
    {
      ...
      $data["SESSION_TIME"] = sprintf(
        "%02d:%02d:%02d",
        $t / 3600,
        ($t / 60) % 60,
        $t % 60
      );
      $data["permissions"]["pages"] = CAT_Users::checkPermission(
        "pages",
        "pages"
      )
        ? true
      ...
      $data["system_information"] = [
        ...
        [
          ...
          "status" =>
            "~ " .
            sprintf("%0.2f", (memory_get_usage() - $_be_mem) / (1024 * 1024)) .
            " MB",
        ],
        [
          "name" => $self->lang()->translate("Script run time"),
          "status" =>
            "~ " . sprintf("%0.2f", microtime(true) - $_be_time) . " sec",
        ],
      ];
    ...
    public static function initPaths()
    {
      ...
      $parser->setFallbackPath(
        CAT_THEME_PATH . "/templates/default",
        "backend"
      );
      ...
      if (file_exists(CAT_THEME_PATH . "/templates/default")) {
        $parser->setPath(CAT_THEME_PATH . "/templates/default", "backend");
        if (
          !CAT_Registry::exists("DEFAULT_THEME_VARIANT") ||
