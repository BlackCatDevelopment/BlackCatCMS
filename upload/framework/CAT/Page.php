  ...
  class CAT_Page extends CAT_Object
  {
    ...
    private static function init($page_id)
    {
      ...
      if (SEARCH == "public") {
        ...
        SEARCH == "private" and
        CAT_User::getInstance()->is_authenticated() == true
      ) {
        CAT_Registry::register("SHOW_SEARCH", true, true);
      } elseif (
        SEARCH == "registered" and
        CAT_User::getInstance()->is_authenticated() == true
      ) {
        CAT_Registry::register("SHOW_SEARCH", true, true);
      } else {
      ...
      if (FRONTEND_LOGIN) {
        ...
        $parser->setGlobals([
          "username_fieldname" => CAT_Helper_Validate::getInstance()->createFieldname(
            "username_"
          ),
          "password_fieldname" => CAT_Helper_Validate::getInstance()->createFieldname(
            "password_"
          ),
          "redirect_url" =>
    ...
    public function show()
    {
      ...
      if (
        CAT_Helper_Page::isMaintenance() ||
        CAT_Registry::get("MAINTENANCE_PAGE") == $page_id
      ) {
        ...
        if (
          file_exists(
            CAT_Helper_Directory::sanitizePath(
              CAT_PATH . "/modules/blackcatFilter/filter.php"
            )
          )
        ) {
    ...
    public function getDescription()
    {
      ...
      if (!$desc) {
        $desc = CAT_Registry::get("WEBSITE_DESCRIPTION");
      }
      return $desc;
    ...
    public function getKeywords()
    {
      ...
      if (!$kw) {
        $kw = CAT_Registry::get("WEBSITE_KEYWORDS");
      }
      return $kw;
    ...
    public function getLanguageMenu()
    {
      ...
      if (isset($items) && count($items)) {
        ...
        if ($parser->hasTemplate("languages")) {
          $parser->output("languages", ["items" => $items]);
        }
      }
    ...
    public function getPageContent($block = 1)
    {
      ...
      if (
        ...
        !CAT_Users::is_root() &&
        (!self::$helper->isMaintenance() ||
          CAT_Registry::get("MAINTENANCE_PAGE") != $this->_page_id)
      ) {
        if (self::$helper->isDeleted($this->_page_id)) {
          ...
          if (FRONTEND_LOGIN) {
            $redirect = $_SERVER["REDIRECT_QUERY_STRING"]
            ...
            header(
              "Location: " .
                LOGIN_URL .
                "?redirect=" .
                CAT_Helper_Validate::getURI(CAT_URL . "/" . $redirect)
            );
            exit();
          } else {
      ...
      if (!self::$helper->isActive($this->_page_id)) {
        return self::$helper
          ->lang()
          ->translate("The page does not have any content!");
      }
      ...
      if (!defined("PAGE_CONTENT") or $block != 1) {
        ...
        if (is_array($sections) && count($sections)) {
          ...
          foreach ($sections as $section) {
            self::$helper
              ->log()
              ->logDebug("sections for this block", $sections);
            $section_id = $section["section_id"];
            $module = $section["module"];
            // make a anchor for every section.
            if (defined("SEC_ANCHOR") && SEC_ANCHOR != "") {
              echo '<a class="section_anchor" id="' .
                SEC_ANCHOR .
                $section_id .
                '"' .
                (isset($section["name"]) && $section["name"] != "no name"
                  ? ' title="' . $section["name"] . '"'
                  : "") .
                "></a>";
            }
            ...
            if (file_exists(CAT_PATH . "/modules/" . $module . "/view.php")) {
              ...
              $langfile = CAT_Helper_Directory::sanitizePath(
                CAT_PATH .
                  "/modules/" .
                  $module .
                  "/languages/" .
              ...
              if (file_exists($langfile)) {
                ...
                if ($this->lang()->checkFile($langfile, "LANG", true)) {
                  $this->lang()->addFile(
                    LANGUAGE . ".php",
                    CAT_Helper_Directory::sanitizePath(
                      CAT_PATH . "/modules/" . $module . "/languages"
                    )
                  );
                }
              }
              ...
              if (
                file_exists(
                  CAT_Helper_Directory::sanitizePath(
                    CAT_PATH . "/modules/" . $module . "/templates"
                  )
                )
              ) {
                $parser->setPath(
              ...
              if (
                file_exists(
                  CAT_Helper_Directory::sanitizePath(
                    CAT_PATH . "/modules/" . $module . "/templates/default"
                  )
                )
              ) {
                $parser->setPath(
    ...
    public function setTemplate()
    {
      if (!defined("TEMPLATE")) {
        ...
        if (isset($prop["template"]) && $prop["template"] != "") {
          if (
            file_exists(
              CAT_PATH . "/templates/" . $prop["template"] . "/index.php"
            )
          ) {
    ...
    {
      if (
        CAT_Registry::defined("ERR_PAGE_404") &&
        CAT_Registry::get("ERR_PAGE_404") != ""
      ) {
        header(
          "Location: " .
            self::$helper->getLink(CAT_Registry::get("ERR_PAGE_404"))
        );
      } else {
    ...
    private function showMenuLink()
    {
      ...
      $tpid = self::$helper
        ...
        ->query(
          sprintf(
            "SELECT * FROM `%smod_menu_link` " . "WHERE `page_id` = %d",
            CAT_TABLE_PREFIX,
            $this->_page_id
          )
        );
      if ($tpid->rowCount() == 1) {
        ...
        if ($target_page_id == -1) {
          ...
          $target_page = $target_page_link = self::$helper->properties(
            $target_page_id
          );
