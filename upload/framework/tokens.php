...
function addTokens( &$html, &$sf) {
	if (!TOKEN_LIFETIME) return;
	//$sf = new SecureCMS();
	$token = $sf->getToken();
	$token1 = "$1?ctoken=$token$3";  		// no parameters so far
	$token2 = "ctoken=$token";  			// for replacing placeholder in JS functions
	$token3 = "$1&amp;ctoken=$token$3";  	// with existing parameters, produces html-valid code
	$token4 = "$1?ctoken=$token$2";		// for special cases
	...
	$qs = '^(send_testmail\(\''. CAT_ADMIN_URL. '/settings/ajax_testmail\.php)(\'\))^';
	$html = preg_replace($qs, $token4, $html, -1);

	// finds forms with method=get and adds a hidden field
	$qs = '^type="hidden" name="ctoken" value=""^';
	//$qs = '^(<form\s+action=[\'"][\w:\.\?/]+ctoken=\w\w\w[\'"]\s+method=[\'"]get[\'"]\s*>)^';
	$html = preg_replace($qs, $hiddentoken, $html, -1);
	
	// set ctoken in JS functions
	$qs = '^leptokh=#-!ctoken-!#^';
