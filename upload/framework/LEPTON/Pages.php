    ...
    class LEPTON_Pages extends LEPTON_Object
    {
        ...
        public function getBackendFooters()
        {
            ...
            if (count(LEPTON_Pages::$js_search_path))
            {
                foreach (LEPTON_Pages::$js_search_path as $directory)
                {
                    ...
                    {
                        LEPTON_Pages::$f_js[] = '<script type="text/javascript" src="' . sanitize_url(LEPTON_URL . $file) . '"></script>' . "\n";
                    }
                }
            }
        ...
        public function getBackendHeaders($section)
        {
            ...
            if (isset($_REQUEST['tool']))
            {
                ...
                if (file_exists($path))
                {
                    array_push(LEPTON_Pages::$css_search_path, '/modules/' . $_REQUEST['tool'], '/modules/' . $_REQUEST['tool'] . '/css');
                    array_push(LEPTON_Pages::$js_search_path, '/modules/' . $_REQUEST['tool'], '/modules/' . $_REQUEST['tool'] . '/js');
        ...
        public function getFrontendFooters()
        {
            ...
            if (count(LEPTON_Pages::$js_search_path))
            {
                foreach (LEPTON_Pages::$js_search_path as $directory)
                {
                    ...
                    if (file_exists(LEPTON_PATH . '/' . $file))
                    {
                        LEPTON_Pages::$f_js[] = $this->space . '<script type="text/javascript" src="' . sanitize_url(LEPTON_URL . $file) . '"></script>' . "\n";
                    }
                }
            }
        ...
        public function getCSS()
        {
            ...
            if (count(LEPTON_Pages::$css))
            {
                foreach (LEPTON_Pages::$css as $item)
                {
                    ...
                    $file = (preg_match('#' . LEPTON_URL . '#i', $item['file']) ? $item['file'] : LEPTON_URL . '/' . $item['file']);
        ...
        private function _analyze_javascripts(&$arr, $for = 'header', $path_prefix = NULL, $section = false)
        {
            ...
            {
                $static[] = $this->space . '<script type="text/javascript" src="' . sanitize_url(LEPTON_URL . '/' . $arr) . '"></script>';
            }
        ...
        private function _analyze_jquery_components(&$arr, $for = 'frontend', $section = NULL)
        {
            ...
            {
                $static[] = $this->space . '<link rel="stylesheet" type="text/css" href="' . sanitize_url(LEPTON_URL . '/modules/lib_jquery/jquery-ui/themes/' . $arr['ui-theme'] . '/jquery-ui.css') . '" media="all" />' . "\n";
            }
            ...
            if (isset($arr['individual']) && is_array($arr['individual']) && isset($section) && $section != '')
            {
                foreach ($arr['individual'] as $section_name => $item)
                {
                    if ($section_name == strtolower($section))
                    {
                        ...
                        $static[] = $this->space . '<script type="text/javascript" src="' . sanitize_url(LEPTON_URL . '/modules/lib_jquery/plugins/' . $item) . '"></script>' . "\n";
                    }
                }
            }
        ...
        private function _load_js($for = 'frontend')
        {
            if (count(LEPTON_Pages::$js_search_path))
            {
                foreach (LEPTON_Pages::$js_search_path as $directory)
                {
                    ...
                    if (file_exists(LEPTON_PATH . '/' . $file))
                    {
                        LEPTON_Pages::$js[] = '<script type="text/javascript" src="' . sanitize_url(LEPTON_URL . $file) . '"></script>' . "\n";
                    }
                }
            }
            ...
            if ($page_id && is_numeric($page_id))
            {
                ...
                if (is_array($sections) && count($sections))
                {
                    ...
                    foreach ($sections as $section)
                    {
                        ...
                        array_push(LEPTON_Pages::$css_search_path, '/modules/' . $module, '/modules/' . $module . '/css');
                        array_push(LEPTON_Pages::$js_search_path, '/modules/' . $module, '/modules/' . $module . '/js');
                    } // foreach ($sections as $section)
                } // if (count($sections))
            }
        }
