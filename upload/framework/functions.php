...
if (!defined('FUNCTIONS_FILE_LOADED'))
{
    ...
    /**
     * Create directories incl. index.php recursive
     *
     * @param string   $dir_name - directory to create
     * @param ocatal   $dir_mode - access mode
     * @return boolean result of operation
     *
     */
    function make_dir_index( $dir_name, $dir_mode = OCTAL_DIR_MODE)
    {
        if (!is_dir($dir_name))
        {
    		make_dir( $dir_name, $dir_mode );

    		change_mode( $dir_name );
    		// Needs to be replaced with function create_access_file() or similar
    		if( is_writable( $dir_name ) )
    		{
    		    // Create default "index.php" file   
    		    $rel_pages_dir	= str_replace( LEPTON_PATH . MEDIA_DIRECTORY, '', dirname( $dir_name ) );
