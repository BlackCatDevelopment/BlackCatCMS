				...
				<div class="fc_page_link{if !$is_editable} fc_page_tree_not_editable{/if}">
					...
					<a href="{if !$is_editable}#{else}{$CAT_ADMIN_URL}/pages/{$action|default:''}.php?page_id={$page_id}{/if}" title="{translate('Page title')}: {$page_title}" class="{if $current}fc_gradient3{/if}">
