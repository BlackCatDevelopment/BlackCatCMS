	...
	<div class="fc_module_block fc_active">
		<form action="{{CAT_ADMIN_URL}}/pages/intro2.php" method="post">
            <div class="fc_settings_max">
			    <input type="radio" class="fc_radio_jq fc_toggle_element" name="intro_forward_by" id="fc_intro_forward_by_disabled" value="disabled" {% if values.intro_forward_by_disabled === true %} checked="checked"{% endif %}>
			    ...
			    <input type="radio" class="fc_radio_jq fc_toggle_element show___fc_forward_by_domain" name="intro_forward_by" id="fc_intro_forward_by_domain" value="domain" {% if values.intro_forward_by_domain === true %} checked="checked"{% endif %}>
			    ...
			    <input type="radio" class="fc_radio_jq fc_toggle_element show___fc_forward_by_lang" name="intro_forward_by" id="fc_intro_forward_by_lang" value="lang" {% if values.intro_forward_by_lang === true %} checked="checked"{% endif %}>
            ...
            {% for domain, page in domains %}
                <div>
                    ...
                    <label class="fc_label_200" for="page_{{$.foreach.default.index}}">{{translate('Page')}}:</label>
                    <select name="pages[]" id="fc_pages_{{$.foreach.default.index}}">
                    {% for lang, items in pages %}
                        <optgroup label="{{lang}}">
                        {% for item in items %}
                            <option value="{{item.page_id}}"{% if item.page_id == page %} selected="selected"{% endif %}>{{item.menu_title}}</option>
                        {% endfor %}
                        </optgroup>
                    {% endfor %}
                    </select><br><hr>
            ...
            {% endfor %}
                ...
                {% for lang, item in pages %}
                    <optgroup label="{{lang}}">
                    {% for item in items %}
                        <option value="{{item.page_id}}"{% if item.page_id == default %} selected="selected"{% endif %}>{{item.menu_title}}</option>
                    {% endfor %}
                    </optgroup>
                {% endfor %}
                </select><br><hr>
            ...
            <div id="fc_forward_by_lang"{% if values.intro_forward_by_lang == 'false' %} style="display:none;"{% endif %}>
            {% for lang, item in pages %}
