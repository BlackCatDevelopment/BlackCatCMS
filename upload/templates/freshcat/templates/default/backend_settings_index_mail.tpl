            <p>
            {{translate('Please specify a default "FROM" address and "SENDER" name below. It is recommended to use a FROM address like: <strong>admin@yourdomain.com</strong>. Some mail provider (e.g. <em>mail.com</em>) may reject mails with a FROM: address like <em>name@mail.com</em> sent via a foreign relay to avoid spam.<br><br>The default values are only used if no other values are specified by Black Cat CMS. If your server supports <acronym title="Simple mail transfer protocol">SMTP</acronym>, you may want use this option for outgoing mails.')|raw}}
			</p>
			<div class="clear"></div>
			<hr>

            {% if CATMAILER_LIBS|length > 0 %}
            <span class="fc_label_200">{{translate('PMailer library')}}:</span><br>
            <div id="fc_catmailer_lib" class="settings_label fc_settings_max">
                {% for item in CATMAILER_LIBS %}
                    <input type="radio" class="fc_radio_jq" name="catmailer_lib" id="fc_catmailer_{{item.dir}}" value="{{item.dir}}" {% if values.catmailer_lib == item.dir %}checked="checked"{% endif %}/>
			...
			<label class="fc_label_200" for="fc_list_SERVER_email">{{translate('PDefault "from" mail')}}:</label>
			<input type="text" name="server_email" id="fc_list_SERVER_email" value="{{values.server_email}}">
			<div class="clear"></div>

			<label class="fc_label_200" for="fc_catmailer_default_sendername">{{translate('PDefault sender name')}}:</label>
            ...
            {% if values.catmailer_lib and values.server_email != "admin@yourdomain.tld" %}
            <div id="testmail_result" style="display:none;">&nbsp;</div>
			<div class="clear"></div>
			<button class="icon-mail right" dir="ltr" type="submit" id="fc_checkmail" name="checkmail" onclick="send_testmail('{{CAT_ADMIN_URL}}/settings/ajax_testmail.php');return false;"> {{translate('PSend test mail')}}</button>
            {% else %}
			...
			<div class="settings_label fc_settings_max" id="fc_catmailer_routine">
				<input type="radio" class="fc_radio_jq fc_toggle_element hide___fc_smtp" name="catmailer_routine" id="fc_catmailer_routine_phpmail" value="phpmail" {% if values.catmailer_routine == 'phpmail' %} checked="checked"{% endif %}/>
				<label for="fc_catmailer_routine_phpmail">{{translate('PHP mail')}}</label>
				<input type="radio" class="fc_radio_jq fc_toggle_element show___fc_smtp" name="catmailer_routine" id="fc_catmailer_routine_smtp" value="smtp" {% if values.catmailer_routine == 'smtp' %} checked="checked"{% endif %}/>
				<label for="fc_catmailer_routine_smtp">{{translate('SMTP')}}</label>
			</div>
			<div class="clear"></div>

			<div id="fc_smtp"{% if values.catmailer_routine == 'phpmail' %} style="display:none;"{% endif %}>
				<hr>
				<p>
					{{translate('<strong>SMTP Mailer Settings:</strong><br>The settings below are only required if you want to send mails via <acronym title="Simple mail transfer protocol">SMTP</acronym>. If you do not know your SMTP host or you are not sure about the required settings, simply stay with the default mail routine: PHP MAIL.')|raw}}
				</p>
				<div class="clear"></div>
				<hr>
                ...
                <span style="font-weight:900">{{translate('Transport security')}}</span>
					<input type="checkbox" class="fc_checkbox_jq fc_toggle_element show___fc_smtp_ssl" name="catmailer_smtp_ssl" id="fc_catmailer_smtp_ssl" value="true" {% if values.catmailer_smtp_ssl and values.catmailer_routine=='smtp' %} checked="checked"{% endif %}>
                    ...
                    <input type="checkbox" class="fc_checkbox_jq fc_toggle_element show___fc_smtp_starttls" name="catmailer_smtp_starttls" id="fc_catmailer_smtp_starttls" value="true" {% if values.catmailer_smtp_starttls and values.catmailer_routine=='smtp' %} checked="checked"{% endif %}>
					...
					<input class="fc_input_small" style="float:right;" type="text" name="catmailer_smtp_ssl_port" id="fc_catmailer_smtp_ssl_port" value="{% if values.catmailer_smtp_ssl_port %}{{values.catmailer_smtp_ssl_port}}{% endif %}"><br>
				</div>
				<div class="clear"></div>

				<div class="fc_settings_max">
					<input type="checkbox" class="fc_checkbox_jq fc_toggle_element show___fc_smtp_aut" name="catmailer_smtp_auth" id="fc_catmailer_smtp_auth" value="true" {% if values.catmailer_smtp_auth and values.catmailer_routine=='smtp' %} checked="checked"{% endif %}/>
				...
				<div id="fc_smtp_aut"{% if values.catmailer_routine == 'phpmail' %} class="fc_inactive_element"{% endif %}>
				<hr>
