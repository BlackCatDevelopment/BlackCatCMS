...
<div id="fc_footer" class="fc_gradient1 fc_border">
	...
	{% endif %}
	<div id="fc_content_footer" {% if not permissions.pages %} class="fc_no_sidebar"{% endif %}>
		<!-- Please note: the below reference to the GNU GPL should not be removed, as it provides a link for users to read about warranty, etc. -->
		<a href="{{URL_HELP}}" title="Black Cat CMS" target="_blank">Black Cat CMS</a> is released under the <a href="https://www.gnu.org/licenses/gpl.html" title="Black Cat CMS Core is GPL" target="_blank">GNU General Public License</a>.
		...
		<div id="fc_footer_info">
			...
			<ul class="fc_gradient1 fc_border fc_shadow_small fc_br_top">
				<li><span>{{CAT_CORE}}:</span> {{CAT_VERSION}} {% if CAT_BUILD %}(Build {{CAT_BUILD}}){% endif %}</li>
				<li><span>{{THEME_NAME}}:</span> {{THEME_VERSION}}</li>
                <li><span>{{translate('Website title')}}:</span> {{WEBSITE_TITLE}}</li>
				{% for info in system_information %}
