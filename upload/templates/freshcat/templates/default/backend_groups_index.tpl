...
<div id="fc_content_header">
    ...
    <div class="fc_header_buttons">
        {% if permissions.USERS %}<a href="{{CAT_ADMIN_URL}}/users/index.php" class="fc_br_left fc_gradient1 fc_gradient_hover">{{translate('Manage users')}}</a>{% endif %}
        <a href="{{CAT_ADMIN_URL}}/groups/index.php" class="{% if not permissions.USERS %}fc_br_all {% else %}fc_br_right {% endif %}fc_gradient1 fc_gradient_hover fc_active">{{translate('Manage groups')}}</a>
    </div>
    <div class="clear"></div>
</div>
...
<div id="fc_main_content">
    <div id="fc_lists_overview">
        ...
        <div class="fc_gradient1 fc_border">
            {% if permissions.GROUPS_ADD %}<button id="fc_list_add" class="icon-plus fc_cell_one fc_gradient1 fc_gradient_hover" title="{{translate('Add group')}}"></button>{% endif %}
            <div class="clear"></div>
        </div>
        ...
        <form action="{{CAT_ADMIN_URL}}/groups/ajax_save_group.php" method="post" id="fc_Group_form" class="fc_list_forms">
            ...
            <div class="fc_toggle_tabs fc_settings_max_large" id="fc_tabs_members">
                ...
                <div id="fc_members" class="fc_settings_max">
                    {% if members %}{{members}}{% else %}{{translate('No members')}}{% endif %}
                </div>
            </div>
            ...
            <div class="clear_sp"></div>
            <p class="submit_settings fc_gradient1">
                {% if permissions.GROUPS_DELETE %}<input type="submit" id="fc_removeGroup" class="fc_modifyGroup fc_list_remove fc_gradient_red" name="removeGroup" value="{{translate('Delete group')}}">{% endif %}
                <input type="submit" name="addGroup" value="{{translate('Add group')}}" class="fc_addGroup">
                <input type="submit" name="saveGroup" value="{{translate('Save group')}}" class="fc_modifyGroup">
                <input type="reset" name="reset" value="{{translate('Reset')}}">
            </p>
