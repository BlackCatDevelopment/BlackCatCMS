...
<div id="fc_main_content">
	...
	{% if $blocks_counter|length > 0 %}
	<ul id="fc_all_blocks">
		{% for block in blocks %}
		<li class="fc_module_block fc_active" id="sectionid_{{block.section_id}}">
			{% if MANAGE_SECTIONS %}<div class="fc_section_drag fc_border_all_light fc_gradient1 fc_gradient_hover icon-move fc_br_top"></div>{% endif %}
			<div class="fc_module_content fc_shadow_small fc_br_all">
										...
										<div class="fc_section_modify_div fc_gradient1 fc_border_all_light fc_shadow_small fc_br_all">
											...
											<form method="post" action="{{CAT_ADMIN_URL}}/pages/ajax_sections_save.php" name="modify_section" class="fc_modify_section">
												<div>
													...
													<div class="clear"></div>
													<hr>
                                                    {% if block.template_blocks|length > 0 %}
													<span class="fc_section_modify_label left">{{translate('Block')}}:</span>
													<select name="set_block" class="left">
													{% for template_block in block.template_blocks %}
														<option value="{{template_block.VALUE}}"{% if template_block.SELECTED %}  selected="selected"{% endif %}>{{template_block.NAME}}</option>
													{% endfor %}
													</select>
													<div class="clear"></div>
													<hr>
				...
				<div class="fc_blocks_footer fc_br_bottom fc_gradient1 fc_border">
                    {% if block.modified_by %}{{translate('Last modification by')}} {{block.modified_by}}{% endif %}
                    {% if block.modified_when %}- {{block.modified_when}}{% endif %}
                </div>
			</div>
		</li>
		{% endfor %}
