				...
				<div class="fc_page_link{if !$editable} fc_page_tree_not_editable{/if}">
					{if $is_parent}<span class="fc_toggle_tree">+</span>{/if}
					<a href="{if !$editable}#{else}{$CAT_ADMIN_URL}/pages/modify.php?page_id={$page_id}{/if}" title="{translate('Page title')}: {$page_title}"{if $current} class="fc_gradient3"{/if}>
