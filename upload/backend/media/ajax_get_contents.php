...
if (
    $val->sanitizePost("load_url") == "" ||
    $user->checkPermission("media", "media_view", false) !== true
) {
...
$fsPath = mb_detect_encoding($load_path, "UTF-8", true)
    ? mb_convert_encoding($load_path, "ISO-8859-1", "UTF-8")
    : $load_path;

if (is_dir($load_path) || is_dir($fsPath)) {
    $ajax["is_folder"] = true;
    ...
    if (isset($dir["filename"]) && is_array($dir["filename"])) {
        foreach ($dir["filename"] as $counter => $file) {
            ...
            $moddate = $dirh->getModdate($file_path);

            $ajax["files"][] = [
                "filetype" => $filetype,
                "show_preview" => in_array($filetype, $allowed_img_types),
                "filesize" => $dirh->getSize($file_path, true),
                "filedate" => $date->strftime(
                    $date->getDefaultDateFormatShort(),
                    $moddate
                ),
                "filetime" => $date->strftime(
                    $date->getDefaultTimeFormat(),
                    $moddate
                ),
                "full_name" => $file,
    ...
    $moddate = $dirh->getModdate($load_path);
    $load_file =
        IS_WIN && function_exists("mb_convert_encoding")
            ? mb_convert_encoding($load_file, "ISO-8859-1", "UTF-8")
            : $load_file;
    ...
    $ajax["files"] = [
        ...
        "filetime" =>
            $date->strftime($date->getDefaultTimeFormat(), $moddate) .
            (isset($language_time_string) ? " " . $language_time_string : ""),
        "full_name" => $load_file,
        "filename" => substr($load_file, 0, -(strlen($filetype) + 1)),
        "load_url" => $val->sanitize_url(
            IS_WIN
                ? utf8_decode(CAT_URL . "/" . $load_url)
                : CAT_URL . "/" . $load_url
        ),
    ];
}
...
// =================================
$ajax["permissions"]["media_upload"] = $user->checkPermission(
    "media",
    "media_upload",
    false
);
$ajax["permissions"]["media_create"] = $user->checkPermission(
    "media",
    "media_create",
    false
);
$ajax["permissions"]["media_rename"] = $user->checkPermission(
    "media",
    "media_rename",
    false
);
$ajax["permissions"]["media_delete"] = $user->checkPermission(
    "media",
    "media_delete",
    false
);
