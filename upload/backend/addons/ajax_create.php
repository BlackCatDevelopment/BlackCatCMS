...
$files_needed = [
    ...
    "library" => ["index.php", "install.php", "upgrade.php"],
    "tool" => [
        "index.php",
        "install.php",
        "upgrade.php",
        ...
        "css/backend.css",
        "languages/DE.php",
        "templates/default/tool.tpl",
    ],
    "template" => ["index.php", "templates/default/index.tpl"],
    "wysiwyg" => ["index.php", "c_editor.php"],
];
$dirs_needed = [
    "page" => ["languages", "templates", "templates/default", "css", "js"],
    "library" => ["vendor"],
    "tool" => ["languages", "templates", "templates/default", "css", "js"],
    "template" => ["languages", "templates", "templates/default", "css", "js"],
    "wysiwyg" => [],
];
$info_prefix = [
    "module" => "module_",
...
// ----- create directories -----
if ($type != "language") {
    $full = CAT_Helper_Directory::sanitizePath(
        CAT_PATH . "/" . $type_path[$func] . "/" . $dir
    );
    // base
...
// ----- create extra files -----
if (in_array($type, ["module", "template"]) && $headinc == "Y") {
    $files_needed[$func][] = "headers.inc.php";
}
if (in_array($type, ["module", "template"]) && $footinc == "Y") {
    ...
    foreach ($files_needed[$func] as $file) {
        ...
        if ($fh) {
            if (
                $ext !== "css" &&
                $ext != "tpl" &&
                substr_compare("languages", $file, 0, 9)
            ) {
            ...
            if (
                ...
                $func =
                    "code_for_" .
                    str_replace(".", "_", pathinfo($file, PATHINFO_BASENAME));
                $func($fh, $type);
            }
            fclose($fh);
        }
...
function module_create_info($full, $type, $name, $dir, $func, $desc, $author)
{
    ...
    module_create_writeHeader($fh, $name, $author, $type);
    foreach ($info as $key => $value) {
        if ($type == "language" && $key == "language_directory") {
            continue;
        }
        fwrite($fh, '$' . $key . ' = "' . $value . '";' . "\n");
    }
