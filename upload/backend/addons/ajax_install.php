...
$backend = CAT_Backend::getInstance('Addons', 'addons', false, false);
...
if (!isset($_FILES['userfile']) || $_FILES['userfile']['size'] == 0)
{
    if(isset($_FILES['userfile']) && isset($_FILES['userfile']['error']))
    {
        ...
        echo CAT_Object::json_error($error);
    }
	echo CAT_Object::json_error('No file');
}
...
{
    echo CAT_Object::json_error('Unable to write to the target directory');
}
...
{
    echo CAT_Object::json_success('Installed successfully');
}
else
{
    $msg = ob_get_clean();
    echo CAT_Object::json_error($msg);
}
