...
$data_dwoo['URL'] = array(
	'MODULES'		=> ADMIN_URL . '/modules/index.php',
...
if ($result->numRows() > 0)
{
	while ($addon = $result->fetchRow())
	{
		...
		if (function_exists('file_get_contents') && file_exists(WB_PATH . '/modules/' . $addon['directory'] . '/languages/' . LANGUAGE . '.php'))
		{
			// read contents of the module language file into string
		    $description             = @file_get_contents(WB_PATH . '/modules/' . $file . '/languages/' . LANGUAGE . '.php');
		...
		// ================================================== 
		if(file_exists(WB_PATH.'/modules/'.$addon['directory'].'/icon.png')){
			list($width, $height, $type, $attr) = getimagesize(WB_PATH.'/modules/'.$addon['directory'].'/icon.png');

			// Check whether file is 32*32 pixel and is an PNG-Image
			$data_dwoo['modules'][$counter]['ICON'] = ($width == 32 && $height == 32 && $type == 3) ?
				WB_URL.'/modules/'.$addon['directory'].'/icon.png' :
				false;
		}
		...
		$data_dwoo['modules'][$counter]['TYPE'] = $type_name;

		// Check if the module is installable or upgradeable
		$data_dwoo['modules'][$counter]['INSTALL']		= file_exists(WB_PATH . '/modules/' . $addon['directory'] . '/install.php') ? true : false;
		$data_dwoo['modules'][$counter]['UPGRADE']		= file_exists(WB_PATH . '/modules/' . $addon['directory'] . '/upgrade.php') ? true : false;
...
foreach ($module_files as $index => $path)
{
	if ( is_dir($path) && empty($modules_count[basename($path)]) )
	{
		...
		{
			$data_dwoo['modules'][$counter]['NAME']			= basename($path);
			$data_dwoo['modules'][$counter]['DIRECTORY']	= basename($path);
			$data_dwoo['modules'][$counter]['INSTALL']		= true;
		}
		/*
		When modules are not already installed, they shouldn't be upgradeable
		if (file_exists($path . '/'))
		{
			$data_dwoo['modules'][$counter]['UPGRADE'] = array(
				'directory'		=> basename($path),
				'name'			=> basename($path)
			);
			$data_dwoo['modules'][$counter]['UPGRADE'] = true;
		}*/
		$counter++;
...
// print page
$parser->output( 'backend_modules_index.lte', $data_dwoo );
