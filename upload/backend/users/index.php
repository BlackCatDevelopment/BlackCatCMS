...
// =============================================
$tpl_data["users"] = $users->getUsers();

// ===========================
// ! Add permissions to Dwoo
// ===========================
$tpl_data["permissions"]["USERS_ADD"] = $users->checkPermission(
    "access",
    "users_add"
)
    ? true
    : false;
$tpl_data["permissions"]["USERS_MODIFY"] = $users->checkPermission(
    "access",
    "users_modify"
)
    ? true
    : false;
$tpl_data["permissions"]["USERS_DELETE"] = $users->checkPermission(
    "access",
    "users_delete"
)
    ? true
    : false;
$tpl_data["permissions"]["GROUPS"] = $users->checkPermission("access", "groups")
    ? true
    : false;
...
$tpl_data["USERNAME"] = isset($_SESSION["au"]["username"])
    ? $_SESSION["au"]["username"]
    : false;
$tpl_data["PASSWORD"] = isset($_SESSION["au"]["password"])
    ? $_SESSION["au"]["password"]
    : false;
$tpl_data["DISPLAY_NAME"] = isset($_SESSION["au"]["display_name"])
    ? $_SESSION["au"]["display_name"]
    : false;
$tpl_data["EMAIL"] = isset($_SESSION["au"]["email"])
    ? $_SESSION["au"]["email"]
    : false;
$tpl_data["HOME_FOLDERS"] = HOME_FOLDERS;
$tpl_data["INITIAL_PAGE"] = INITIAL_PAGE;
$tpl_data["NEWUSERHINT"] = preg_split(
    "/, /",
    $backend
        ->lang()
        ->translate(
            ...
            [
                "name" => CAT_Registry::get("AUTH_MIN_LOGIN_LENGTH"),
                "password" => CAT_Registry::get("AUTH_MIN_PASS_LENGTH"),
            ]
        )
);
...
foreach (directory_list(CAT_PATH . MEDIA_DIRECTORY) as $index => $name) {
    $tpl_data["home_folders"][$index]["NAME"] = str_replace(
        CAT_PATH,
        "",
        $name
    );
    $tpl_data["home_folders"][$index]["FOLDER"] = str_replace(
        CAT_PATH . MEDIA_DIRECTORY,
        "",
        $name
    );
}
