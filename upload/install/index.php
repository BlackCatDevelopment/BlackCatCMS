...
function fill_tables($database) {
    ...
    $database->query($secrows);
	if ( $database->is_error() ) {
		trigger_error(sprintf('[%s - %s] %s', __FILE__, __LINE__, $database->get_error()), E_USER_ERROR);
		$errors['secrows'] = $database->get_error();
	}
	else {
	    fwrite( $logh, 'filled table [class_secure]'."\n" );
	}
...
function create_default_page($database) {
    ...
    $pg_content = "<?php
/**
 *	This file is autogenerated by LEPTON Version 2.0 \"Black Cat Edition\" Installer
